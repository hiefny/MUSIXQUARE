...
type StagerCreateBlobSource = NonNullable<
  FilePlaybackAssetSourceStagerRuntimeForTests['createBlobSource']
>;
type StagerCreateEncodedSource = NonNullable<
  FilePlaybackAssetSourceStagerRuntimeForTests['createEncodedSource']
>;
type StagerStageCandidate = NonNullable<
  FilePlaybackAssetSourceStagerRuntimeForTests['stageCandidate']
>;
type StagerRetireCandidate = NonNullable<
  FilePlaybackAssetSourceStagerRuntimeForTests['retireCandidate']
>;
...
function managerAudioGraph(): {
  ...
  return { context, destination: gate };
}
...
function successfulRuntime(
  ...
  const createBlobSource = vi.fn<StagerCreateBlobSource>(async () => result);
  const createEncodedSource = vi.fn<StagerCreateEncodedSource>(async () => result);
  const stageCandidate = vi.fn<StagerStageCandidate>(async (_manager, options) => {
    await options.source.connect(options.destination);
    return PORT as never;
  });
  const retireCandidate = vi.fn<StagerRetireCandidate>(async () => true);
  return {
    runtime: {
      createBlobSource,
      createEncodedSource,
...
describe('revision-free file playback warm source', () => {
  ...
  it('joins retirement to an in-flight manager handoff and retires only its resolved port', async () => {
    ...
    const retireCandidate = vi.fn<StagerRetireCandidate>(async () => true);
    const stageCandidate = vi.fn<StagerStageCandidate>(() => pendingStage.promise);
    const runtime = {
...
describe('stageFilePlaybackAssetSource', () => {
  ...
  it('destroys and releases a factory result superseded while its factory was pending', async () => {
    ...
    let current = true;
    const stageCandidate = vi.fn<StagerStageCandidate>(async () => PORT as never);
  ...
  it.each(['forged', 'retired'] as const)(
    ...
    async (mode) => {
      ...
      const createBlobSource = vi.fn<StagerCreateBlobSource>(async () =>
        factoryResult(fakeCutoverSource()),
      );
