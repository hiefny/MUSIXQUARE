...
import { memoryReservationStatsForTests } from '../decode-admission.ts';
import {
  decodeOrdinaryAudio,
...
describe('decodeOrdinaryAudio', () => {
  it('skips predictive metadata rejection for unknown-duration iOS input', async () => {
    ...
    const result = await decodeOrdinaryAudio(request(blob, audioContext(decode)));
  ...
  it('keeps measured PCM accounting without predictively rejecting the legacy path', async () => {
    ...
    expect(result.audioBuffer).toBe(oversized);
    expect(memoryReservationStatsForTests().decodeBytes).toBeGreaterThan(0);
    result.release();
  });
