...
interface ForgedGeometryMutation {
...
describe('MP3 decoder helpers', () => {
  ...
  const forgedGeometryCases: ReadonlyArray<
    readonly [label: string, mutate: (metadata: Readonly<Mp3Metadata>) => ForgedGeometryMutation]
  > = [
    [
      'source size',
  ...
  it.each(forgedGeometryCases)('fails closed on forged %s metadata geometry', (_label, mutate) => {
    const metadata = metadataFixture();
