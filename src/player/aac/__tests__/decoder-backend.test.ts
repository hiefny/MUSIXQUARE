...
describe('AAC decoder PCM batch snapshot', () => {
  it('deeply detaches exact stereo PCM while preserving transferable plane ownership', () => {
    ...
    const [left, right] = snapshot.planes;
    if (!left || !right) throw new Error('Expected detached stereo PCM planes');
    expect(Object.isFrozen(left)).toBe(false);
    expect(left).not.toBe(sourcePlanes[0]);
    expect(left.buffer).not.toBe(sourcePlanes[0].buffer);
    expect(right.buffer).not.toBe(sourcePlanes[1].buffer);

    const retained = left[0];
    sourcePlanes[0].fill(0.75);
    sourcePlanes[1].fill(-0.75);
    expect(left[0]).toBe(retained);
    expect(right[0]).not.toBe(-0.75);
  });
  ...
  it('accepts the maximum bounded mono and stereo geometries', () => {
    for (const mono of [true, false]) {
      ...
      for (const plane of snapshot.planes) totalPlaneBytes += plane.byteLength;
      expect(totalPlaneBytes).toBe(8_192 * (mono ? 1 : 2) * Float32Array.BYTES_PER_ELEMENT);
    }
  });
