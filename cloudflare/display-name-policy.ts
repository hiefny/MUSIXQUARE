 ...
 */
export function hasDisplayNameWhitespaceOrFiller(value: unknown): boolean {
  return (
    typeof value === 'string' &&
...
function isAllowedVariationSelector(character: string, previousCharacter: string | undefined) {
  return (
...
function allowedEmojiTagIndexes(characters: readonly string[]): Set<number> {
  ...
  for (let index = 0; index < characters.length; index += 1) {
    ...
    while (cursor < characters.length) {
      ...
      if (character === undefined || !EMOJI_TAG_CHARACTER_RE.test(character)) break;
      cursor += 1;
    }
...
function isUnsafeDisplayNameCharacter(
  character: string,
  previousCharacter: string | undefined,
  index: number,
  allowedTags: ReadonlySet<number>,
): boolean {
  if (
 ...
 */
export function sanitizeDisplayNameForValidation(value: unknown): string {
 ...
 */
export function displayNameSecuritySkeleton(value: unknown): string {
  if (typeof value !== 'string') return '';
...
export function hasVisibleDisplayNameContent(value: unknown): boolean {
 ...
 */
export function isSafeVisibleDisplayName(value: unknown): boolean {
