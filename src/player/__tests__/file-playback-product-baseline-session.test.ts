...
): Extract<FilePlaybackApplicationLifecycleEvent, { kind: 'revoked' }>;
function lifecycle(
  kind: 'clock-degraded' | 'clock-ready' | 'established',
  role: 'host' | 'guest',
  connectionValue: DataConnection,
  channel: FilePlaybackConnectionChannel,
): Exclude<FilePlaybackApplicationLifecycleEvent, { kind: 'revoked' }>;
function lifecycle(
  ...
  if (!channel) throw new Error(`Lifecycle ${kind} requires an established channel`);
  return Object.freeze({ kind, role, connection: connectionValue, channel });
}
...
describe('FilePlaybackProductBaselineSession', () => {
  ...
  it('emits a canonical stopped baseline without inventing active media identity', () => {
    ...
    const sent = vi.fn((_connection: DataConnection, _frame: unknown) => true);
    const session = machine({
  ...
  it('completes baseline and READY exactly once in both lifecycle orderings', () => {
    const pair = channelPair();
    const hostSent = vi.fn((_connection: DataConnection, _frame: unknown) => true);
    const guestSent = vi.fn((_connection: DataConnection, _frame: unknown) => true);
