...
| Sync/clock           | `src/network/sync.ts`, `heartbeat-monitor.ts`, `shared-clock.ts`, `sync-worker.ts`, `workers/sync.worker.ts`                                    | `sync.ts` owns offsets, latency history, ping state, and clock samples; `heartbeat-monitor.ts` owns per-connection liveness leases and stale-peer cleanup. | `SYNC_PING`, `SYNC_PONG`, `sync:*`, `worker:timer-tick`, `state:setup.sessionStarted`, peer-disconnect publication. | Dedicated worker timer with main-thread fallback; WebRTC connection and channel state.             | `resetSyncClockRuntime`, `stopWorkerTimer`, heartbeat monitor stop, `detachHostPeerConnection`, sessionCode/hostConn listeners. | Background throttling can starve application pings. Closed or unknown transports use an 8s threshold, recovering transports 30s, and fully connected transports 90s.       |
...
| Remote share         | `src/share/remote-share.ts`, `remote-upload.ts`                                                                                                | Owns `share.remote.*` progress and remote descriptors; writes file track meta on accepted remote file.            | `REMOTE_FILE_SHARE`, `REMOTE_FILE_UNAVAILABLE`, `share:remote-file`, `remote-file:*`.                    | Private remote object storage endpoint and fetch.                                                | AbortController per upload/download, descriptor checks, sessionCode reset.                                           | First remote whole file for a late guest finishing after host already played. Sync bootstrap now covers part of this, but e2e coverage is still the proof gap.             |
...
| OP guest seek/play/pause/chat command                   | Guest requests host action; host verifies OP before mutation/broadcast.                                                                  | `player/playback.ts` request handlers, `room-control.ts` delegated chat handling, protocol capability verification.           | Peer disconnect clears rate buckets and host peer state.                                     | Playback, room-control, protocol, and chat tests.                                               | Low-medium: UI can show controls, but host remains authority.                  |
...
| Sync                     | `network/__tests__/sync.test.ts`, `heartbeat-monitor.test.ts`, `room-control.test.ts`, `sync-worker.test.ts`, worker tests | SYNC_PONG mode/activity, clock reset, exact-connection heartbeat leases, 8/30/90s grace, cleanup ordering, room-control authority, fallback timers. | Mobile lock-screen and worker throttling on real devices. |
...
| Remote share             | `share/__tests__/remote-share.test.ts`                                                                     | Policy and descriptor acceptance/rejection.                                     | Fetch/object-storage integration and whole-object download timeout behavior. |
...
2. Remote whole-file late-join path: descriptor arrives, download finishes after host already played, immediate sync bootstrap starts playback.
...
- Add remote whole-object first-file late-join integration test.
