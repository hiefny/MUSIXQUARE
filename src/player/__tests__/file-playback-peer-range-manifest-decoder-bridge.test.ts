...
interface AcquiredFixture {
  ...
  readonly transport: PeerRangeTransport & { readonly closeHandle: ReturnType<typeof vi.fn> };
}
...
describe('peer-range manifest decoder construction bridge', () => {
  it('keeps ADTS evidence private, uses one snapshotted Worker seam, and consumes once', async () => {
    ...
    let canaryRuntime: AacWorkerCapabilityProbeRuntime | null = null;
    ...
    expect('scan' in (wrapperOptions as unknown as Record<string, unknown>)).toBe(false);
    expect(wrapperOptions?.runtime?.createWorker).toBe(canaryRuntime?.createWorker);
    expect(wrapperOptions?.runtime?.createWorker?.()).toBe(originalWorker);
