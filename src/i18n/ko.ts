...
const ko = {
  ...
  'connect.kicked_message': '호스트에 의해 세션에서 내보내졌어요.',
  ...
  'toast.host_ended_connection': '호스트에서 연결이 종료되었어요',
  ...
  // ─── Legal ────────────────────────────────────────────────────────
  'legal.title': '저작권 · 개인정보 · 면책',
  'legal.content_html': '<strong>저작권 안내</strong><br>뮤직스퀘어를 통해 공유되는 음악, 영상 등의 콘텐츠에 대한 저작권은 해당 권리자에게 있어요. 시스템 오디오 공유 및 파일 공유 기능은 개인적인 사적 이용을 위한 것이며, 저작권이 있는 콘텐츠의 무단 배포 및 공개 송신에 대한 책임은 사용자에게 있어요.<br><br><strong>개인정보</strong><br>뮤직스퀘어는 P2P(기기 간 직접 연결) 방식으로 작동해요. 오디오, 파일, 채팅 데이터는 서버를 거치지 않고 기기 간에 직접 전송되며, 서버에 저장되지 않아요. 다만 기기 간 연결을 위해 IP 주소가 시그널링 서버를 경유할 수 있어요. 이 정보는 연결 수립 목적으로만 사용되며, 별도로 수집하거나 보관하지 않아요.<br><br><strong>면책 조항</strong><br>뮤직스퀘어는 무료로 제공되는 서비스로, 서비스의 지속적인 운영이나 안정성을 보장하지 않아요. 모바일 데이터 환경에서 사용 시 데이터 요금이 발생할 수 있으며, 서비스 이용 중 발생하는 문제에 대해 개발자는 책임을 지지 않아요.',
} as const;
