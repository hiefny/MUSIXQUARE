...
describe('AacDecoderAdapter', () => {
  ...
  it('makes a postMessage-reentrant close join the exact retirement until delayed ACKs arrive', async () => {
    ...
    const worker = h.workers[0];
    if (!worker) throw new Error('Expected reentrant AAC close');
    const close = requireAssigned<Promise<void>>(
      () => reentrantClose,
      'Expected reentrant AAC close',
    );
  ...
  it('bounds a postMessage-reentrant close when its physical source read never settles', async () => {
    ...
    try {
      ...
      const worker = h.workers[0];
      if (!worker) throw new Error('Expected stalled reentrant AAC close');
      const close = requireAssigned<Promise<void>>(
        () => reentrantClose,
        'Expected stalled reentrant AAC close',
      );
      let closeSettled = false;
      void close.then(() => {
        closeSettled = true;
  ...
  it('publishes the retirement barrier before generation listener removal can re-enter close', async () => {
    ...
    const close = requireAssigned<Promise<void>>(
      () => reentrantClose,
      'Expected removeEventListener-reentrant AAC close',
    );
    let closeSettled = false;
    void close.then(() => {
      closeSettled = true;
