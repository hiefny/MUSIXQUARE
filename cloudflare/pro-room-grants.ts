...
interface CampaignRow extends JsonRecord {
  campaign_id: string;
  slug: string;
  status: string;
}

interface AccountSession extends JsonRecord {
  accountId: string;
}

interface DigestedVoucher {
  roomCode: string;
  codeDigest: string;
}

interface PreparedVoucher extends DigestedVoucher {
  voucherId: string;
  roomGeneration: number;
}
...
interface D1DatabasePort {
  ...
  batch?: (statements: D1StatementPort[]) => Promise<unknown>;
}

type ReadJsonResult =
  | { value: JsonRecord; error?: never }
  | { error: 'JSON_REQUIRED' | 'INVALID_REQUEST' | 'REQUEST_TIMEOUT'; value?: never };
...
function hasD1Database(value: unknown): value is D1DatabasePort {
  return isRecord(value) && typeof value.prepare === 'function';
}

async function executeBatch(db: D1DatabasePort, statements: D1StatementPort[]): Promise<unknown> {
  if (typeof db.batch !== 'function') throw new TypeError('D1 batch unavailable');
...
function isAccountSession(value: unknown): value is AccountSession {
  return isRecord(value) && typeof value.accountId === 'string';
}

function hasPrepare(value: unknown): boolean {
  return isRecord(value) && typeof value.prepare === 'function';
}
...
async function keyedDigest(secret: unknown, domain: string, value: string): Promise<string | null> {
...
function normalizeVoucherCode(value: unknown): string | null {
  const normalized =
    typeof value === 'string'
      ? value
          .trim()
...
async function first(statement: D1StatementPort): Promise<JsonRecord | null> {
  const value =
    typeof statement.first === 'function'
      ? await statement.first()
      : property(await statement.all(), 'results');
  const row = Array.isArray(value) ? value[0] : value;
  return isRecord(row) ? row : null;
}

async function all(statement: D1StatementPort): Promise<JsonRecord[]> {
  const result = await statement.all();
  const rows = property(result, 'results');
  return Array.isArray(rows) ? rows.filter(isRecord) : [];
}

function publicVoucherMappings(rows: readonly JsonRecord[]) {
  return rows.map((row) => ({
...
function changeCount(result: unknown): number {
  const meta = property(result, 'meta');
  return Number(property(meta, 'changes') ?? property(result, 'changes') ?? 0);
}
...
async function readJson(request: Request, maxBytes = MAX_BODY_BYTES): Promise<ReadJsonResult> {
  ...
  try {
    ...
    return isRecord(value) ? { value } : { error: 'INVALID_REQUEST' };
  } catch {
    return { error: 'INVALID_REQUEST' };
  }
}
...
function campaignPublicState(row: JsonRecord, nowMs: number): CampaignPublicState;
function campaignPublicState(row: null, nowMs: number): null;
function campaignPublicState(row: JsonRecord | null, nowMs: number): CampaignPublicState | null;
function campaignPublicState(row: JsonRecord | null, nowMs: number): CampaignPublicState | null {
  if (!row) return null;
  ...
  if (status === 'active' && endsAt !== null && Number.isSafeInteger(endsAt) && endsAt <= nowMs) {
    status = 'ended';
  }
  return {
    slug: row.slug,
...
function campaignVoucherSummary(row: JsonRecord | null) {
  ...
  const firstRoomCode =
    typeof row?.first_room_code === 'string' && ROOM_CODE_RE.test(row.first_room_code)
      ? row.first_room_code
      : null;
  const lastRoomCode =
    typeof row?.last_room_code === 'string' && ROOM_CODE_RE.test(row.last_room_code)
      ? row.last_room_code
      : null;
  return {
...
async function readCampaignSummary(db: D1DatabasePort, campaignId: string) {
  return first(
    db
      .prepare(
...
async function readCampaignList(db: D1DatabasePort) {
  return all(
    db.prepare(
      `${CAMPAIGN_SUMMARY_SELECT}
       GROUP BY campaign.campaign_id
...
function campaignAdminState(row: JsonRecord | null, nowMs: number) {
  return {
...
function campaignStatusTransitionAllowed(
  current: unknown,
  requested: unknown,
  endsAt: number | null,
  nowMs: number,
  ...
  if (requested === 'active') {
    return (
      ...
      ['draft', 'paused'].includes(current) &&
      (endsAt === null || endsAt > nowMs)
    );
  }
  if (requested === 'paused') return current === 'active' && (endsAt === null || endsAt > nowMs);
  if (requested === 'ended') {
    return typeof current === 'string' && ['draft', 'active', 'paused'].includes(current);
  }
  return false;
}

async function readCampaign(db: D1DatabasePort, slug: string) {
  ...
  return isCampaignRow(row) ? row : null;
}

async function readAccountRedemption(db: D1DatabasePort, campaignId: string, accountId: string) {
  return first(
    db
      .prepare(
...
function normalizedRedemption(row: JsonRecord | null) {
  if (
    !row ||
    typeof row.redemption_id !== 'string' ||
    typeof row.grant_id !== 'string' ||
    typeof row.redemption_status !== 'string' ||
...
function proEntitlementDb(env: unknown): D1DatabasePort | null {
  const candidate = property(env, 'MUSIXQUARE_ADMIN_DB') || property(env, 'ADMIN_METRICS_DB');
  return hasD1Database(candidate) ? candidate : null;
}

function validEntitlementIdentity(input: unknown, accountKey = 'accountId'): boolean {
  return (
    ACCOUNT_ID_RE.test(String(property(input, accountKey) || '')) &&
    ROOM_CODE_RE.test(String(property(input, 'roomCode') || '')) &&
    isProRoomGeneration(property(input, 'roomGeneration'))
  );
}

function entitlementNow(input: { nowMs?: number }): number {
  return Number.isSafeInteger(input.nowMs) && (input.nowMs ?? -1) >= 0
    ? (input.nowMs ?? 0)
    : Date.now();
}

async function readExactEntitlement(db: D1DatabasePort, input: ProGrantActivationIdentity) {
  return first(
    db
      .prepare(
...
async function accountHasCurrentProEntitlement(db: D1DatabasePort, accountId: string) {
  const row = await first(
    db
      .prepare(
        `SELECT 1 AS current
...
async function hasCompletedOwnerEntitlementBackfill(db: D1DatabasePort) {
  const row = await first(
    db.prepare(
...
export async function markProRoomOwnerEntitlementBackfillComplete(
  env: unknown,
  nowMs = Date.now(),
): Promise<boolean> {
...
export async function canAccountReceiveProRoomEntitlement(
  env: unknown,
 ...
 */
export async function reserveProRoomActivationEntitlement(
  env: unknown,
  ...
  try {
    ...
    if (
      existing &&
      typeof existing.source_kind === 'string' &&
    ...
    try {
      ...
      return (
        typeof raced?.source_kind === 'string' &&
        ['grant', 'legacy_activation'].includes(raced.source_kind) &&
...
export async function finalizeProRoomActivationEntitlement(
  env: unknown,
  input: ProGrantActivationIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
  ...
  try {
    ...
    return (
      typeof finalized?.source_kind === 'string' &&
...
export async function finalizeProGrantActivation(
  env: unknown,
  input: ProGrantActivationIdentity,
): Promise<boolean> {
...
export async function upsertProRoomOwnerEntitlement(
  env: unknown,
 ...
 */
export async function canOrphanProRoomOwnerEntitlement(
  env: unknown,
  input: ProGrantActivationIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
 ...
 */
export async function orphanProRoomOwnerEntitlement(
  env: unknown,
  input: ProGrantActivationIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
  ...
  try {
    ...
    if (typeof exact.source_ref !== 'string') return false;
...
async function readOwnershipTransferTarget(
  db: D1DatabasePort,
...
function ownershipTransferTargetMatches(
  row: JsonRecord | null,
  input: ProRoomOwnershipTransferEntitlementIdentity,
): boolean {
  return (
...
export async function reserveProRoomOwnershipTransferEntitlement(
  env: unknown,
  input: ProRoomOwnershipTransferEntitlementIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
...
export async function finalizeProRoomOwnershipTransferEntitlement(
  env: unknown,
  input: ProRoomOwnershipTransferEntitlementIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
...
export async function abortProRoomOwnershipTransferEntitlement(
  env: unknown,
  input: ProRoomOwnershipTransferEntitlementIdentity,
): Promise<boolean> {
  const db = proEntitlementDb(env);
...
export async function revokeProRoomEntitlement(
  env: unknown,
...
export async function orphanAccountProGrants(
  env: unknown,
  accountId: string,
  nowMs = Date.now(),
): Promise<boolean> {
...
async function rotatePendingProGrantLifecycleRow(
  db: D1DatabasePort,
  row: JsonRecord,
  nowMs: number,
): Promise<boolean> {
  ...
  try {
    const touched = await db
      ...
      .run();
    return changeCount(touched) === 1;
  } catch {
    return false;
  }
}

export async function reconcileProGrantLifecycle(
  env: unknown,
  ...
  const limit = Math.max(
    ...
    Math.min(
      100,
      typeof options.limit === 'number' && Number.isSafeInteger(options.limit) ? options.limit : 25,
    ),
  );
...
export async function hasReservedProGrantAllocation(
  env: unknown,
  roomCode: string,
  roomGeneration: number,
): Promise<boolean> {
...
export async function authorizeProGrantActivation(
  env: unknown,
...
async function issueSetupUrl(
  dependencies: ProGrantDependencies,
  accountId: string,
  redemption: NonNullable<ReturnType<typeof normalizedRedemption>>,
) {
  if (!redemption?.setupRequired) return { setupRequired: false };
  if (typeof dependencies.issueActivationHandoff !== 'function') {
...
async function reconcileAccountRedemption(
  env: unknown,
  dependencies: ProGrantDependencies,
  accountId: string,
  row: JsonRecord,
  ...
  if (typeof dependencies.inspectRoom !== 'function') return redemption;
...
async function existingProEntitlementCount(db: D1DatabasePort, accountId: string): Promise<number> {
  const row = await first(
    db
      .prepare(
...
async function redeemVoucher(
  ...
  const db = proEntitlementDb(env);
  if (!db) return responseJson({ error: 'PRO_GRANT_UNAVAILABLE' }, 503);
  ...
  const pepper = String(property(env, 'MXQR_PRO_GRANT_VOUCHER_PEPPER') || '');
  ...
  if (
    typeof voucher.voucher_id !== 'string' ||
    typeof voucher.room_code !== 'string' ||
...
export async function handleProGrantPublicRequest(
  request: Request,
  env: unknown,
  url: URL,
  dependencies: ProGrantDependencies,
  ...
  if (
    !db ||
    !hasPrepare(property(env, 'MUSIXQUARE_AUTH_DB')) ||
    typeof dependencies.resolveAccountSession !== 'function'
  ) {
    return responseJson({ error: 'PRO_GRANT_UNAVAILABLE' }, 503);
  }
  ...
  if (action === 'setup-link') {
    ...
    const setup = await issueSetupUrl(dependencies, account.accountId, redemption);
...
async function batchFingerprint(
  pepper: unknown,
  campaignId: string,
  vouchers: readonly DigestedVoucher[],
): Promise<string | null> {
  return keyedDigest(
    pepper,
...
async function readVoucherBatchReplay(
  db: D1DatabasePort,
  campaignId: string,
  requestId: string,
  requestDigest: string | null,
  expectedCount: number,
): Promise<VoucherBatchReplay> {
...
async function adminVoucherBatch(
  request: Request,
  env: unknown,
  campaign: CampaignRow,
  dependencies: ProGrantDependencies,
  ...
  const normalized: Array<{ roomCode: string; code: string }> = [];
  const seenRooms = new Set<string>();
  for (const item of input) {
    if (!isRecord(item) || Object.keys(item).some((key) => !['roomCode', 'code'].includes(key))) {
      return responseJson({ error: 'INVALID_REQUEST' }, 400);
    }
    const roomCode = typeof item.roomCode === 'string' ? item.roomCode : '';
  ...
  const digested: DigestedVoucher[] = [];
  ...
  for (const item of digested) {
    if (typeof dependencies.preflightVoucherRoom !== 'function') {
  ...
  try {
    ...
    const raced: VoucherBatchReplay = await readVoucherBatchReplay(
      db,
      campaign.campaign_id,
      requestId,
      requestDigest,
      prepared.length,
    ).catch((): VoucherBatchReplay => ({ state: 'unavailable' }));
...
export async function handleProGrantAdminRequest(
  request: Request,
  env: unknown,
  url: URL,
  dependencies: ProGrantDependencies,
  ...
  if (!slug) {
    ...
    if (
      ...
      (startsAt !== null &&
        (typeof startsAt !== 'number' || !Number.isSafeInteger(startsAt) || startsAt < 0)) ||
      (endsAt !== null &&
        (typeof endsAt !== 'number' || !Number.isSafeInteger(endsAt) || endsAt < 0)) ||
      (typeof startsAt === 'number' && typeof endsAt === 'number' && endsAt <= startsAt) ||
      typeof perAccountLimit !== 'number' ||
  ...
  if (action === 'revoke') {
    ...
    if (
      ...
      !SAFE_ADMIN_REASON_RE.test(reason) ||
    ...
    const nowMs = Date.now();
    const actorId = `admin:${requestId}`;
    const result = `revoke-unused:${reason}`;
    const previous = await first(
      db
        .prepare(
          `SELECT result FROM ${AUDIT_TABLE}
    ...
    if (!previous) {
      let results: unknown[];
      try {
        const batchResult = await executeBatch(db, [
          db
            .prepare(
        ...
        results = Array.isArray(batchResult) ? batchResult : [];
      } catch {
        return responseJson({ error: 'PRO_GRANT_UNAVAILABLE' }, 503);
      }
      const committed = await first(
