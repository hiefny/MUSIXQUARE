...
export const PRODUCTION_WRANGLER_CONFIGS: readonly string[] = Object.freeze([
...
function normalizeRepositoryPath(root: string, path: string): string {
...
export function discoverProductionWranglerConfigs(root = repositoryRoot): string[] {
...
export function assertProductionWranglerConfigCoverage(root = repositoryRoot): string[] {
...
export function readPinnedWranglerToolchain(root = repositoryRoot): {
  ...
  const devDependencies = isRecord(packageJson) ? packageJson.devDependencies : undefined;
  const pinnedVersion = isRecord(devDependencies) ? devDependencies.wrangler : undefined;
  if (
  ...
  if (installedVersion !== pinnedVersion) {
    throw new Error(
      `Installed Wrangler ${String(installedVersion)} does not match pinned ${pinnedVersion}. Run npm ci.`,
    );
  }
  const installedBin = isRecord(installedPackage) ? installedPackage.bin : undefined;
  const binaryPath = isRecord(installedBin) ? installedBin.wrangler : undefined;
...
export function workerBundleDryRunEnvironment(
...
export function runWorkerBundleDryRuns(root = repositoryRoot): void {
