...
describe('HostRendezvousCoordinator', () => {
  ...
  it('cannot commit arm or finalize receipts after Proxy inspection re-enters cancellation', async () => {
    ...
    const finalize = vi.fn((_intent: RendezvousFinalizeIntent) => finalizeGate.promise);
  ...
  it('returns immediately, uses the slowest estimate, and never lets a slow peer block peers', async () => {
    ...
    const observed = { healthyFinalizeIntent: null as RendezvousFinalizeIntent | null };
    ...
    const healthyFinalize = vi.fn(async (intent: RendezvousFinalizeIntent) => {
      observed.healthyFinalizeIntent = intent;
    ...
    expect(observed.healthyFinalizeIntent?.startAtRoomTimeMs).toBe(attempt.startAtRoomTimeMs);
    expect(Object.isFrozen(healthyArmIntent)).toBe(true);
    expect(Object.isFrozen(observed.healthyFinalizeIntent)).toBe(true);
  ...
  it('retires armed old targets without awaiting cancel success', async () => {
    ...
    const oldCancel = vi.fn<NonNullable<HostRendezvousParticipant['cancel']>>(() =>
      Promise.reject(new Error('peer already disconnected')),
    );
  ...
  it('does not cancel an explicitly committed participant when a newer attempt supersedes it', async () => {
    ...
    const commitAttempt = vi.fn<NonNullable<HostRendezvousParticipant['commitAttempt']>>(
      () => true,
    );
