...
import { recordPeerHeartbeat } from './heartbeat-monitor.ts';
...
function isSyncPongTrackIdentityCurrent(data: Record<string, unknown>): boolean {
  if (getState('demo.active')) {
    ...
    return (
      Number.isSafeInteger(demoTrackIndex) &&
      demoTrackIndex === getState('demo.currentTrackIndex')
    );
  }
...
export function initSync(): void {
  ...
  bus.on('state:network.hostConn', () => {
    if (getState('network.appRole') === 'guest') resetSyncClockRuntime();
  });
