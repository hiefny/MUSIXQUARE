...
function normalizeSameOriginAssetUrl(value, baseUrl = `${APP_ORIGIN}/`) {
...
function withParsedDocument(html, inspect) {
...
function isActiveHtmlElement(element) {
...
export function extractMainScript(html) {
  return withParsedDocument(html, (document) => {
    ...
    return mainScripts.length === 1 ? mainScripts[0] : null;
  });
}

function collectEagerAssetUrls(html) {
  return withParsedDocument(html, (document) => {
    const urls = [];
    const moduleUrls = [];
    const moduleEntries = [];
    ...
    return {
      entryScriptUrl: moduleEntries[0],
      urls: [...new Set(urls)],
...
function staticModuleImports(source, importerUrl) {
...
async function candidateAssetIdentity(assetUrl, read) {
...
export async function expectedAppAssetGraph({ read = readFile } = {}) {
  ...
  const ensureIdentity = async (assetUrl) => {
...
export async function expectedMainScript({ read = readFile } = {}) {
...
export async function expectedMainAsset({ mainScript, read = readFile } = {}) {
...
async function digestBoundedBody(response, maxBytes) {
  const declaredLength = declaredContentLength(response);
  if (declaredLength !== null && declaredLength > maxBytes) {
    cancelBody(response.body);
    throw new Error(`response exceeds ${maxBytes} bytes`);
...
export async function readPublicIndex({ timeoutMs }) {
...
function validAssetContentType(assetUrl, contentType) {
...
export async function readPublicAsset({ assetUrl, timeoutMs }) {
...
export async function readPublicMainAsset({ mainScript, timeoutMs }) {
