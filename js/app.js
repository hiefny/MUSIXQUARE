...
function checkSystemCompatibility() {
    ...
    setTimeout(() => {
        if (!isSecure) {
            ...
            log.info(`[Compatibility] iOS detected.`);
        }
    }, 1500);
}
...
function clearAllManagedTimers() {
    ...
    // (All preload watchdog timers are now tracked via managedTimers.preloadWatchdog)
}

let channelMode = 0; // 0=Stereo, -1=Left, 1=Right, 2=Sub
// Toss in-app: 역할(채널 모드)은 Settings에서 변경 가능해야 합니다.
...
const BlobURLManager = {
    ...
    safeRevoke(url, options) {
        ...
        if (this._pendingRevocations.size >= this.MAX_PENDING) {
            ...
            // Check if attached before immediate revocation
            if (this._isUrlAttached(oldest)) {
                this._deferredUntilDetached.add(oldest);
                this._pendingRevocations.delete(oldest);
                log.debug(`[BlobURL] Queue full. Deferring oldest (still attached): ${oldest}`);
            } else {
...
let _syncBusyRetryCount = 0; // Track sync busy retries locally (not on wire)
...
// _preloadWatchdog consolidated into managedTimers.preloadWatchdog
...
// Guest Side
// preloadChunks removed: vestigial from pre-Worker architecture
let preloadCount = 0;
...
let globalSessionCounter = Math.floor(Date.now() / 1000) + Math.floor(Math.random() * 100000); // Robust Session ID Start
...
function clearPreloadState() {
    ...
    if (preloadMeta && preloadMeta.total > 0) {
        ...
        if (progress > 0.9 && progress < 1.0) {
            log.warn(`[Preload] State reset - almost complete (${Math.round(progress * 100)}%). Continuing clean.`);
        }
    }
...
const handleWorkerMessage = async (e) => {
    try {
        ...
        if (data.type === 'TICK') {
            ...
            log.warn(`[Worker-OPFS] Write error for ${data.filename} chunk ${data.chunk}:`, data.error);
        }
            ...
            if (data.type === 'OPFS_READ_ERROR') {
                ...
                const sepIdx = reqStr.lastIndexOf('|');
                const peerId = sepIdx > 0 ? reqStr.slice(0, sepIdx) : reqStr;
                const tag = sepIdx > 0 ? reqStr.slice(sepIdx + 1) : '';
                if (tag === 'catchup') {
...
async function finalizeFileProcessing(file) {
    ...
    try {
        ...
        videoElement.addEventListener('loadedmetadata', () => {
            // Use currentAudioBuffer (live reference) instead of closed-over audioBuffer
            // to avoid stale duration during rapid track switching
            const buf = currentAudioBuffer || audioBuffer;
            const seekSlider = document.getElementById('seek-slider');
            const timeDur = document.getElementById('time-dur');
            if (seekSlider) { seekSlider.max = buf.duration; seekSlider.value = 0; }
            if (timeDur) timeDur.innerText = fmtTime(buf.duration);
...
// --- Language / i18n extracted to js/i18n.js ---
...
// setupShowInstruction removed: no #setup-instruction element in HTML
...
function initSetupOverlay() {
    ...
    const showAndStart = () => {
        ...
        try { freezeLayoutMetricsOnce(); } catch (_) { /* ignore */ }
        showSetupOverlay();
        startObAutoSlide();
    };

    // On the very first app launch, add boot-block class
...
// btn-role-back removed: back button is now rendered dynamically via setupRenderActions
...
function initEventListeners() {
    ...
    $on('btn-setup-join', 'click', () => { handleSetupJoinWithRole(pendingGuestRoleMode); });
...
async function play(offset) {
    ...
    try {
        ...
        setTimeout(() => {
            _isPlayLocked = false;
            // Drain queued play request if one was stored while locked
            if (_pendingPlayTime !== undefined) {
                const pendingOffset = _pendingPlayTime;
                _pendingPlayTime = undefined;
                play(pendingOffset);
            }
        }, 10);
...
function setEngineMode(mode) {
    ...
    // [Key Fix] If we were idle or paused, don't force transition to PLAYING.
    // This prevents "ghost playback" UI where state is PLAYING but nothing has started.
    // We move to PAUSED instead; the subsequent play() call will set the final PLAYING state.
    if (isIdleOrPaused(currentState)) {
        setState(APP_STATE.PAUSED);
    } else {
...
function toggleSurroundMode(enabled) {
    ...
    if (!enabled) setChannelMode(channelMode); // Restore standard channel only if surround is disabled
...
function setSurroundChannel(idx, el) {
...
async function setChannel(mode, el, force = false, notify = true) {
    ...
    // Scope active class removal to the parent grid of the clicked element
    const parentGrid = el ? el.closest('.channel-grid') : null;
    (parentGrid || document).querySelectorAll('.ch-opt').forEach(e => e.classList.remove('active'));
    if (el) el.classList.add('active');
    setChannelMode(mode);
...
// Individual reverb reset wrappers removed: unused (resetReverb calls setReverbParam directly)
...
function applySettings() {
    ...
    if (reverb) {
        ...
        if (needsGenerate) {
            reverb.generate().catch(e => log.warn('[Reverb] generate() failed:', e));
        }
    }
    if (rvbLowCut) {
...
if (slider) {
    ...
    slider.addEventListener('change', () => {
        ...
        if (isActuallyPlaying) {
            ...
            pausedAt = t;
            if (currentState === APP_STATE.PLAYING_VIDEO && videoElement) videoElement.currentTime = t;
...
// updateSyncBtnState removed: dead code, never called
...
function joinSession(retryAttempt = 0, hostIdOverride = null) {
    ...
    conn.on('open', () => {
        ...
        // Message handler (must wrap to pass conn — PeerJS only sends data)
...
async function leaveSession(opts = {}) {
    ...
    // Transfer State Flags (prevent stale state on rejoin)
    _skipIncomingFile = false;
    _skipIncomingPreload = false;
    _waitingForRelayData = false;
    transferState = TRANSFER_STATE.IDLE;
    ...
    _preloadUsedForIndex = null;
    window._playPreloadedInProgress = undefined;

    // Global Window State
    _activeBroadcastSession = null;
    _pendingFileName = undefined;
    _pendingFileIndex = undefined;
    _ytIOSWatchdog = null;
    ...
    try { preloadReorderBuffer.clear(); } catch (_) { }
    try { preloadSessionState.clear(); } catch (_) { }
...
// detectConnectionType removed: local-only build, usePingCompensation is already false
...
function clearPreviousTrackState(reason = '') {
    ...
    // Note: We do NOT clear preload state here (nextFileBlob, etc.)
...
async function handleFileChunk(data) {
    ...
    if (data.index < 5 || data.index % 100 === 0) {
        log.debug(`[Chunk] Received idx=${data.index}, total=${meta ? meta.total : '?'}`);
    }
    ...
    if (meta && meta.total > 0) {
        ...
        if (meta.size) {
            ...
            const currentBytes = Math.min(receivedCount * CHUNK_SIZE, meta.size);
    ...
    if (meta && receivedCount >= meta.total && transferState !== TRANSFER_STATE.PROCESSING) {
        ...
        // Cleanup reorder buffer to prevent memory leak across track switches
...
async function handleSyncResponse(data) {
    ...
    if (data.reqTs && data.reqTs > 0) {
        ...
        if (elapsed > 150) {
            ...
            if (_syncBusyRetryCount < 3) {
                log.debug(`[AutoSync] Host was busy (${elapsed}ms round-trip). Retry ${_syncBusyRetryCount}/3 in 300ms...`);
    ...
    if (data.isPlaying) {
        // Guard: only play if we actually have audio data loaded
        const hasAudio = currentAudioBuffer || (currentFileBlob && currentFileBlob.size > 0);
        if (hasAudio) {
            play(compensatedTime + localOffset);
        } else {
            log.debug("[AutoSync] Host is playing but we have no audio data yet. Storing position.");
            pausedAt = compensatedTime + localOffset;
        }
    }
    else {
...
async function handlePlayPreloaded(data) {
    ...
    if (isPreloadTargetMatch) {
        ...
        if (upstreamDataConn && upstreamDataConn.open) {
            ...
            setTimeout(async () => {
                if (!hostConn || !hostConn.open) return;
                // Recheck: did preload arrive during jitter delay?
                if (nextMeta && nextMeta.index === data.index && nextFileBlob) {
                    log.debug("[Guest] Preload arrived during jitter wait! Playing...");
                    await loadPreloadedTrack(data.index, ++_currentLoadToken);
                    return;
                }
...
async function handlePlay(data) {
    ...
    if (data.index !== undefined && hostConn) {
        const expectedName = data.name || (playlist[data.index] && (playlist[data.index].name || playlist[data.index].title));
        // Normalize names for comparison (handle potential [Display Name] vs [File Name] mismatch)
        const normalize = n => String(n || '').toLowerCase().trim().replace(/\[.*?\]\s*/g, '');
        if (expectedName && meta && meta.name && normalize(meta.name) !== normalize(expectedName)) {
            // [Refinement] If index matches current exactly, trust it as canonical source of truth (Issue #10)
            if (data.index === currentTrackIndex) {
                log.warn(`[Guest] Name mismatch for current index ${data.index} (Loaded: "${meta.name}" vs Msg: "${expectedName}"). Proceeding anyway.`);
            } else {
...
function handlePause(data) {
    ...
    if (t !== undefined && Number.isFinite(t)) {
        ...
        if (currentState === APP_STATE.PLAYING_VIDEO && videoElement) {
...
function handleReverb(data) { if (data.value === undefined) return; setReverbParam('mix', data.value); }
async function handleReverbType(data) {
    if (reverb && data.value) {
        const prevDecay = reverbDecay;
        const prevPreDelay = reverbPreDelay;
        if (data.value === 'room') {
            reverb.decay = 1.5;
        ...
        try {
            ...
            reverb.decay = prevDecay;
            reverb.preDelay = prevPreDelay;
            reverbDecay = prevDecay;
            reverbPreDelay = prevPreDelay;
            log.warn('[Reverb] generate() failed during type change, reverted:', e?.message || e);
            showToast('리버브 적용 실패');
            return;
        }
...
function handleReverbDecay(data) { if (data.value === undefined) return; setReverbParam('decay', data.value); }
function handleReverbPreDelay(data) { if (data.value === undefined) return; setReverbParam('predelay', data.value); }
function handleReverbLowCut(data) { if (data.value === undefined) return; setReverbParam('lowcut', data.value); }
function handleReverbHighCut(data) { if (data.value === undefined) return; setReverbParam('highcut', data.value); }

function handleEQUpdate(data) {
    if (data.band === undefined || data.value === undefined) return;
    setEQ(data.band, data.value, false, true);
}

function handlePreamp(data) {
    if (data.value === undefined) return;
    setPreamp(data.value, false, true);
}

function handleEQReset(data) {
    resetEQ(true);
}

function handleStereoWidth(data) {
    if (data.value === undefined) return;
    setStereoWidth(data.value);
}

function handleVBass(data) {
    if (data.value === undefined) return;
...
function handleRepeatMode(data) {
    if (data.value === undefined) return;
    ...
    if (hostConn) {
        ...
        if (downstreamDataPeers.length > 0) {
            if (RELAYABLE_COMMANDS.includes(data.type)) {
                downstreamDataPeers.forEach(p => {
                    // Prevent infinite loop: do not relay back to the connection that sent it
                    if (p.open && p !== conn) {
        ...
        if (conn && conn !== hostConn && hostConn && hostConn.open) {
...
function connectToRelay(targetId) {
    ...
    _relayConnTimer = setTimeout(() => {
        if (!conn.open) {
            ...
            if (hostConn && hostConn.open) {
                const recoveryFileName = _pendingFileName || (meta ? meta.name : '');
                const recoveryIndex = _pendingFileIndex !== undefined ? _pendingFileIndex : currentTrackIndex;
...
// handleManualSync simplified: direct alias
const handleManualSync = openManualSyncUI;
    ...
    conn.on('data', async data => {
        if (data.type === MSG.REQUEST_CURRENT_FILE) {
            ...
            if (isMatchCurrent) {
                ...
                unicastFile(conn, currentFileBlob).catch(e => log.error('[Relay] unicast current failed:', e));
            }
            else if (isMatchPreload) {
                log.debug(`[Relay] Serving preloaded file to ${conn.peer.substr(-4)}: ${nextMeta.name}`);
                unicastFile(conn, nextFileBlob).catch(e => log.error('[Relay] unicast preload failed:', e));
            }
...
function handleOperatorRequest(data, conn) {
    ...
    // Fix: Guard against missing connection (Security)
    if (!conn) return;
    ...
    if (data.type === MSG.REQUEST_PLAY) {
        ...
        if (isNaN(idx) || idx < 0 || idx >= playlist.length) {
            log.warn(`[Host] OP requested invalid track index: ${data.index}`);
            return;
        }
        log.debug("[Host] OP requested track change to:", idx);
        playTrack(idx);
        ...
        const sec = parseFloat(data.sec);
        if (isNaN(sec)) return;
        log.debug("[Host] OP requested skip time:", sec);
        skipTime(sec);
    } else if (data.type === MSG.REQUEST_SEEK) {
        const time = parseFloat(data.time);
        if (isNaN(time) || time < 0) return;
        log.debug("[Host] OP requested seek to:", time);

        if (currentState === APP_STATE.PLAYING_YOUTUBE && youtubePlayer) {
            try {
                youtubePlayer.seekTo(time, true);
                broadcast({ type: MSG.YOUTUBE_STATE, state: youtubePlayer.getPlayerState(), time: time });
            } catch (e) {
        ...
        if (currentState === APP_STATE.PLAYING_VIDEO || currentState === APP_STATE.PLAYING_AUDIO) {
            play(time);
            broadcast({ type: MSG.PLAY, time: time, index: currentTrackIndex });
        } else {
            pausedAt = time;
            if (videoElement) videoElement.currentTime = time;
            broadcast({ type: MSG.PAUSE, time: time });
        }
        ...
        if (data.settingType === MSG.REVERB) { setReverb(data.value); broadcast({ type: MSG.REVERB, value: data.value }); }
            ...
            broadcast({ type: MSG.PREAMP, value: val });
        }
...
function sendRecoveryRequest(forceChunk = null) {
    ...
    setTimeout(() => {
        ...
        // Re-evaluate connection after backoff (original may have been replaced)
        const freshConn = (upstreamDataConn && upstreamDataConn.open) ? upstreamDataConn : hostConn;
        ...
        const latestName = (meta && meta.name) ? meta.name : (_pendingFileName || '');
...
function showToast(msg) {
    ...
    try {
        ...
        clearManagedTimer('toast');
        managedTimers.toast = setTimeout(() => {
            try { t.classList.remove('show'); } catch (_) { /* noop */ }
        }, 2000);
    } catch (e) {
        // Last resort fallback
        log.info('[Toast fallback]', msg);
    }
}
...
function loopUI() {
    ...
    if (isPlaybackState) {
        ...
        const now = Date.now();
        if (!_lastEndedCheck || now - _lastEndedCheck > ENDED_CHECK_THROTTLE) {
            _lastEndedCheck = now;
            handleEnded();
        }
...
async function loadPreloadedTrack(expectedIndex = undefined, loadToken = undefined) {
    ...
    try {
        ...
        // Video load (confirm after load starts)
        if (videoElement) {
            videoElement.load();
        }
...
// updateUISlider removed: dead code, never called
...
function _formatChatDisplayName(label) {
...
function closeYouTubePopup() {
    // Clear pending preview fetch to avoid updating DOM after popup is closed
    if (youtubePreviewDebounce) { clearTimeout(youtubePreviewDebounce); youtubePreviewDebounce = null; }
    animateTransition(() => {
        const ov = document.getElementById('youtube-url-overlay');
        if (ov) ov.classList.remove('active');
        updateOverlayOpenClass();
...
function loadYouTubeVideo(videoId, playlistId = null, autoplay = true, subIndex = 0) {
    ...
    if (!window.YT || !window.YT.Player) {
        if (!_ytScriptLoading) {
            ...
            tag.onload = () => { _ytScriptLoading = false; log.debug('[YouTube] API script loaded'); };
            tag.onerror = () => {
    ...
    _ytLoadTimeout = setTimeout(() => {
        if (_currentYouTubeSessionId === currentSessionId && (!youtubePlayer || !window.isYouTubeAPIReady)) {
...
async function processRelayQueue() {
    ...
    // Release processed entries (modifies in-place to avoid GC pressure from slice)
    if (idx >= relayChunkQueue.length) {
        relayChunkQueue = [];
    } else if (idx > 0) {
        relayChunkQueue.splice(0, idx);
    }
