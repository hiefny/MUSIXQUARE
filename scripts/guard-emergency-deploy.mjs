...
export const EMERGENCY_DEPLOY_TARGETS = Object.freeze([
  'remote-share',
  'pro-room',
...
function isTarget(value) {
  return EMERGENCY_DEPLOY_TARGETS.includes(value);
}

export function expectedEmergencyDeployConfirmation(target, commitSha) {
...
function readLiveOriginMainSha(reader = readGit) {
  try {
...
export function authorizeEmergencyDeploy(target, options = {}) {
...
function rejectDirectDeploy(target) {
...
function main() {
  ...
  if (
    !['authorize', 'reject'].includes(mode) ||
    !target ||
    (mode === 'authorize' && extra.length)
  ) {
    throw new Error('Usage: node scripts/guard-emergency-deploy.mjs <authorize|reject> <target>');
  }
