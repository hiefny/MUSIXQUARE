...
The original audit found **at least seven** partially-overlapping flags
governing "what is this player doing right now?":

- `playback.mode` / `playback.activity` (current decomposed ownership axis)
...
- Coexistence with the playback mode/activity ownership axis
...
## 2. Decomposed playback state

Playback mode/activity identify which subsystem owns output and whether it is
idle, pending, paused, or playing. The file lifecycle state machine describes
the asynchronous local/received-file pipeline. These slots coexist; the old
single `APP_STATE` enum and its `PLAYING_VIDEO` compatibility value have been
removed.

```
PLAYBACK_MODE      : null | file | youtube | system-audio
PLAYBACK_ACTIVITY  : idle | pending | paused | playing
PLAYBACK_LIFECYCLE : IDLE | DOWNLOADING | AWAITING_PRELOAD | DECODING | READY | PLAYING | PAUSED | FAILED
                              ↑ meaningful only for the file pipeline
```

For YouTube and system audio, `playback.lifecycle` stays at IDLE because those
surfaces have their own lifecycle paths. Ownership guards keep the file state
machine from claiming an active external playback surface.
...
| `PAUSE(time)`                                                 | IDLE                             | set pausedAt; publish paused file mode/activity upstream          |
...
| AWAITING_PRELOAD | preload stall (10s, resets on accepted progress)                        | preload stall                                                                |
| DECODING         | none; native `decodeAudioData` is awaited                               | native work is not cancellable; ownership checks discard a superseded result |
| READY            | —                                                                       | —                                                                            |
...
| `transfer.skipIncomingFile`                                                   | **deleted**                 | mutable cross-module state was replaced by `shouldSkipIncomingFile(data)`, derived from lifecycle, playback ownership, exact transfer identity, and the resident blob                                                                          |
...
| YouTube accidentally picks up PLAYBACK_STATE changes         | L          | M      | Ownership guards reject file-lifecycle claims while an external surface owns playback     |
...
| Hidden failure-marking gap in the preload decode-error path  | —          | —      | **FIXED**; current decode-path tests cover failure and supersession branches              |
...
- [ ] Unify file lifecycle with YouTube's separate lifecycle — leave alone
...
| --- | ----------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 1   | Enum name                     | **PLAYBACK_STATE**                                         | pairs with the existing TRANSFER_STATE naming                                                |
...
- `5b1c24e`: historically introduced a 10s decode timeout; the current decoder
  deliberately removed that deadline because `decodeAudioData` cannot be
  cancelled and a timed-out native decode can overlap its retry.
- `fb5d5f0`: repeat/shuffle hardening (Fisher-Yates)
- `6dca780`: sync popup UI tweak
- Historical prep item: fix the `markTrackFailed` gap in the preload-failure
...
_Design verified against comprehensive code audit (2026-04-20). Ready for
Phase 2 implementation._
