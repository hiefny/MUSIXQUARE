...
def replace_once(path: str, before: str, after: str) -> None:
    ...
    if count != 1:
        raise SystemExit(
            f"{path}: expected exactly one patch anchor, found {count}: {before[:120]!r}"
        )
...
liveness_module_path = "src/network/transport/signaling-liveness.ts"
require_missing(liveness_module_path)
write(
    liveness_module_path,
    textwrap.dedent(
        r'''\
        ...
        export const SIGNALING_LIVENESS_TIMEOUT_MS = 15_000;
        ...
        /**
         * Application-level liveness for the exact admitted signaling WebSocket.
         *
         * The transport enables this only for room hosts/coordinators. A single
         * fixed probe per room bounds Cloudflare message volume while still
         * detecting Windows half-open sockets that never emit close or error.
         */
        export class SignalingSocketLivenessMonitor {
          ...
          start(socket: WebSocket): void {
            ...
            const probe: SignalingProbe = {
              intervalKey: `signaling-liveness-${this.monitorId}-${socketId}`,
              timeoutKey: `signaling-liveness-timeout-${this.monitorId}-${socketId}`,
              awaitingSince: null,
              lastTickAt: Date.now(),
            };
          ...
          noteMessage(socket: WebSocket, raw: unknown): boolean {
            ...
            if (probe) {
              clearManagedTimer(probe.timeoutKey);
          ...
          stop(socket: WebSocket): void {
            const probe = this.probes.get(socket);
            if (!probe) return;
            clearManagedTimer(probe.intervalKey);
            clearManagedTimer(probe.timeoutKey);
          ...
          private scheduleTimeout(socket: WebSocket, probe: SignalingProbe): void {
            clearManagedTimer(probe.timeoutKey);
            setManagedTimer(probe.timeoutKey, () => {
              if (this.probes.get(socket) !== probe || probe.awaitingSince === null) return;
              const elapsed = Date.now() - probe.awaitingSince;
              if (elapsed > SIGNALING_LIVENESS_SUSPENSION_GAP_MS) {
                // A suspended/backgrounded page is not evidence of a dead route.
                probe.awaitingSince = null;
                probe.lastTickAt = Date.now();
                return;
              }
              if (elapsed >= SIGNALING_LIVENESS_TIMEOUT_MS) this.fail(socket, probe);
          ...
          private tick(socket: WebSocket, probe: SignalingProbe): void {
            ...
            if (probe.awaitingSince !== null) return;

            try {
              socket.send(SIGNALING_LIVENESS_PING);
              probe.awaitingSince = now;
              this.scheduleTimeout(socket, probe);
            } catch {
              this.fail(socket, probe);
            }
          }
...
liveness_test_path = "src/network/transport/__tests__/signaling-liveness.test.ts"
require_missing(liveness_test_path)
write(
    liveness_test_path,
    textwrap.dedent(
        r'''\
        ...
        class WorkerState {
          ...
          getWebSockets(): WorkerSocket[] { return []; }
        }
...
retirement_methods = textwrap.dedent(
    '''\
...
old_host_close = textwrap.dedent(
    '''\
...
new_host_close = textwrap.dedent(
    '''\
        socket.addEventListener('close', (event) => {
          if (this.destroyed) {
            this.signalingLiveness.stop(socket);
...
replace_once("src/network/transport/cloudflare-signaling.ts", old_host_close, new_host_close)

old_guest_close = textwrap.dedent(
    '''\
        socket.addEventListener('close', (event) => {
          if (this.destroyed) return;
          if ((event as CloseEvent).reason === 'PRO_COORDINATOR_EPOCH_ADVANCED') {
...
new_guest_close = textwrap.dedent(
    '''\
        socket.addEventListener('close', (event) => {
          if (this.destroyed) {
            this.signalingLiveness.stop(socket);
...
replace_once("src/network/transport/cloudflare-signaling.ts", old_guest_close, new_guest_close)
replace_once(
    "src/network/transport/cloudflare-signaling.ts",
...
replace_once("scripts/release-evidence.mts", old_wait_loop, new_wait_loop)
replace_once(
    "scripts/release-evidence.mts",
...
write(
    ...
    textwrap.dedent(
        ...
        - **Applies to:** Cloudflare standard-room and PRO signaling transports

        ## Problem

        Windows Chromium can retain a signaling WebSocket in the `OPEN` state after the
        network route disappears. Existing WebRTC data channels may continue to work,
        but the room can no longer admit a new device. Waiting only for WebSocket
        `close` or `error` therefore leaves the QR invite action showing **Copy invite
        link** while the signaling path is unusable.

        ## Decision

        The signaling Worker advertises protocol version 1 in the authenticated
        `peer-open` frame. Upgraded room hosts/coordinators send one fixed
        application-level ping on the exact admitted WebSocket every ten seconds.
        Cloudflare Durable Objects answer the fixed frame with
        `setWebSocketAutoResponse()` so the room object does not wake for normal probes;
        local and compatibility runtimes retain an explicit fallback response.

        A socket that does not produce a pong or any other server message within fifteen
        seconds is identity-fenced out of transport state before it is closed. The
        existing `disconnected` lifecycle then drives the 1/2/4/8/15-second retry budget
        and `network.signalingHealth`, so the current QR action changes to **Recovering**
        without a second UI state machine. Browser `offline` starts the same recovery
        immediately, and `online` requests an immediate retry.

        ## Cost and compatibility

        - Only the host/coordinator signaling socket is periodically probed, so an
          ordinary room adds at most one probe stream regardless of participant count.
        - Monitoring starts only after the Worker advertises support, so either side of
          a rolling deployment remains compatible.
        - A late pong or close from a superseded socket cannot mutate its replacement.
        - Existing WebRTC data channels and system-audio media are preserved while only
          the signaling socket generation is rebuilt.
        - Long background timer gaps re-arm monitoring instead of declaring a false
          outage.
        - UI layout, copy, and room flows are unchanged.
        - Media loading remains best-effort and RAM-only. The 200 MiB value remains a
          transfer/private-storage protocol ceiling, not a device-memory admission
          limit.
        '''
    ),
)
...
for transient in (
    ...
    "scripts/one-shot-signaling-liveness-patch.py",
...
print("Applied cost-aware signaling liveness repair and removed transient automation.")
