...
function aiffFile(
  ...
  return new File([copyBlobPart(bytes)], options.name ?? 'recording.bin', {
...
function cafFile(
  ...
  return new File(
    [copyBlobPart(concatBytes(header, cafChunk('desc', description), cafChunk('data', data)))],
...
describe('createBlobFilePlaybackSource', () => {
  ...
  it.each([
    [
      'truncated AIFF',
      new File(
        [copyBlobPart(concatBytes(asciiBytes('FORM'), uint32Be(4), asciiBytes('AIFF')))],
        'payload.bin',
        {
          type: 'application/octet-stream',
        },
...
describe('createEncodedFilePlaybackSource', () => {
  ...
  it('snapshots a hostile encodedSource getter once before validation and ownership', async () => {
    ...
    let getterCalls = 0;
    const receivedSource: { current: EncodedAudioSource | null } = { current: null };
    const options = encodedOptions(first.source, {
      backendFactories: {
        createStreamingFlacSource: (sourceOptions) => {
          receivedSource.current = sourceOptions.encodedSource;
    ...
    expect(result.backend).toBe('bounded-stream');
    expect(getterCalls).toBe(1);
    expect(receivedSource.current).not.toBe(first.source);
    expect(receivedSource.current?.identity).toBe(first.source.identity);
  ...
  it('snapshots changing nested source fields once for parsing, decoding, and the result', async () => {
    ...
    const result = await createEncodedFilePlaybackSource(
      encodedOptions(hostileSource, {
        backendFactories: {
          createStreamingLinearPcmSource: (options) => {
            receivedSource.current = options.encodedSource;
            return new StreamingLinearPcmPlaybackSource(options);
          },
        },
      }),
    ...
    expect(Object.is(receivedSource.current, hostileSource)).toBe(false);
    expect(receivedSource.current?.identity).toBe(stableIdentity);
    expect(receivedSource.current?.size).toBe(bytes.byteLength);
    expect(receivedSource.current?.metadata).toEqual({ name: file.name, mime: file.type });
  ...
  it('transfers a canonical view of the exact generic FLAC lease until destroy', async () => {
    ...
    expect(result.backend).toBe('bounded-stream');
    expect(receivedSource.current).not.toBe(fixture.source);
    expect(receivedSource.current?.identity).toBe(fixture.source.identity);
