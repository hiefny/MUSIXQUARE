...
test.describe('universal bounded FLAC over a real host/guest room', () => {
  ...
  test('FLAC streams by peer range with one bounded renderer identity and retires reads', async () => {
    ...
    expect(snapshots.host.renderer?.revision).toBe(snapshots.guest.controller?.timeline.revision);
    expect(snapshots.host.peerRangePhysicalReads.maxRequestByteLength).toBeLessThanOrEqual(
      64 * 1_024,
    );
