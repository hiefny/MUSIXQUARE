...
export interface DurableObjectGitRunnerOptions {
  cwd?: string;
...
export type DurableObjectGitRunner = (
  command: string,
  args: string[],
  options?: DurableObjectGitRunnerOptions,
) => string | Buffer;
...
function assertRepositoryPath(root: string, path: unknown, label: string): asserts path is string {
  if (
    typeof path !== 'string' ||
...
function valueNestingDepth(value: string): number {
  let depth = 0;
  let quote: '"' | "'" | null = null;
  let escaped = false;
...
function parseBasicString(value: string, label: string): string {
  let parsed: unknown;
  try {
...
function isDurableObjectClassName(value: unknown): value is string {
  return (
    typeof value === 'string' && /^[A-Za-z_$][0-9A-Za-z_$]*$/u.test(value) && value.length <= 128
  );
}
...
function parseStringArray(value: string, label: string): string[] {
  ...
  if (
    ...
    parsed.length === 0 ||
    !parsed.every(isDurableObjectClassName) ||
...
function assertMigrationShape(
  migration: unknown,
  label: string,
): asserts migration is DurableObjectMigration {
  ...
  for (const key of keys.filter((key) => key !== 'tag')) {
    const operation = migration[key];
    if (
      !Array.isArray(operation) ||
      operation.length === 0 ||
      !operation.every(isDurableObjectClassName) ||
      new Set(operation).size !== operation.length
    ) {
...
export function parseProductionWranglerMigrations(
  ...
  let inTopLevel = true;
  let currentMigration: Record<string, unknown> | null = null;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTopLevel && assignment?.[1] === 'name') {
      ...
      scriptName = parseBasicString(nameValue, `${label}.name`);
      continue;
    }
    ...
    while (depth > 0 && index + 1 < lines.length) {
      ...
      const continuation = stripTomlComment(continuationLine).trim();
      rawValue += `\n${continuation}`;
  ...
  const validatedMigrations: DurableObjectMigration[] = [];
  ...
  return { scriptName, migrations: validatedMigrations };
}

export function discoverProductionWranglerMigrationConfigs(root = repositoryRoot): string[] {
...
export function loadDurableObjectMigrationManifest(
  root = repositoryRoot,
): DurableObjectMigrationManifest {
  return JSON.parse(
    readFileSync(resolve(root, DURABLE_OBJECT_MIGRATION_MANIFEST_PATH), 'utf8'),
  ) as DurableObjectMigrationManifest;
}

function assertManifestStructure(manifest: DurableObjectMigrationManifest): Set<string> {
  if (
...
}: { root?: string; manifest?: DurableObjectMigrationManifest } = {}): {
  schemaVersion: number;
  configCount: number;
...
export function assertDurableObjectMigrationManifestAppendOnly({
  ...
  const previousConfigs = new Map<string, DurableObjectMigrationConfig>();
  const currentConfigs = new Map<string, DurableObjectMigrationConfig>();
  ...
  for (const [config, previous] of previousConfigs) {
    ...
    for (let index = 0; index < previous.migrations.length; index += 1) {
      const previousMigration = previous.migrations[index];
      const currentMigration = current.migrations[index];
      if (previousMigration === undefined || currentMigration === undefined) {
        throw new Error(`${config} Durable Object migration history cannot be truncated.`);
      }
      if (stableJson(previousMigration) !== stableJson(currentMigration)) {
        throw new Error(
          `${config} Durable Object migration history is append-only; existing tag ${previousMigration.tag} changed.`,
        );
      }
    }
  }
...
function parseHistoricalManifest(
  ...
  try {
    return JSON.parse(String(raw)) as DurableObjectMigrationManifest;
  } catch (error) {
...
export function assertDurableObjectMigrationRepositoryHistory({
  ...
  let previousManifest: DurableObjectMigrationManifest | null = null;
