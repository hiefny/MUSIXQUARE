...
function inertHostRoom(): FilePlaybackProductRuntimeHostRoomPort {
  ...
  return {
    ...
    currentPeerPublication: vi.fn(() => null),
    resolveCurrentPeerRangeSource: unsupported,
...
describe('peer product runtime lifecycle', () => {
  it('owns room lifecycle only through the gate-aware facade', () => {
    ...
    // Retire the facade on transport replacement, failed host-room startup,
    // provisional setup cancellation, and an established-room leave.
    expect(PEER_SOURCE.match(/getFilePlaybackProductRuntime\(\)\.endRoom\(\)/gu)).toHaveLength(4);
  });
