...
const channelStageUnpreparedBaselineMedia =
  FilePlaybackConnectionChannel.prototype.stageUnpreparedBaselineMedia;
...
export class FilePlaybackConnectionMediaSession {
  ...
  stageRunBinding(
    ...
    return this.#mutate(() => {
      ...
      try {
        channelStateLease = Reflect.apply(
          kind === 'baseline' ? channelStageUnpreparedBaselineMedia : channelStageMedia,
          this.#channel,
          [
            freezeCanonical({
              run: expectedState,
  ...
  commitAdmittedStop(
    ...
    return this.#mutate(() => {
      ...
      const operation = this.#requireOperation(preparedOperation);
      const unpreparedBaseline =
        operation === this.#candidate &&
        operation.status === 'candidate' &&
        operation.kind === 'baseline' &&
        this.#current === null &&
        this.#currentState === null;
      const prepared = unpreparedBaseline ? operation : this.#requireCurrent(preparedOperation);
      if (
        this.#candidateState ||
        (unpreparedBaseline
          ? !sameState(prepared.binding, expected)
          : !this.#currentState || !sameStateIdentity(this.#currentState, expected)) ||
        expected.queueItemId !== prepared.binding.queueItemId ||
        expected.runId !== prepared.binding.runId ||
        next.queueItemId !== expected.queueItemId ||
        next.runId !== expected.runId ||
      ...
      const readCommitAuthority = (): boolean =>
        unpreparedBaseline
          ? this.#readUnpreparedBaselineCommitAuthority(prepared, isStillCurrent)
          : this.#readPreparedCommitAuthority(prepared, isStillCurrent);
      if (!readCommitAuthority()) {
      ...
      try {
        if (unpreparedBaseline) {
          this.#runAuthority.commitUnpreparedBaselineStop(
            this.#connectionToken,
            prepared.runLease,
            expected,
            next,
  ...
  #readUnpreparedBaselineCommitAuthority(
    record: OperationRecord,
    isStillCurrent: () => boolean,
  ): boolean {
    this.#assertOperationCurrent(record);
    if (record.kind !== 'baseline' || this.#current !== null || this.#currentState !== null) {
      throw new Error('File playback unprepared baseline is no longer exact candidate authority');
    }
    let accepted: boolean;
    try {
      accepted = Reflect.apply(isStillCurrent, undefined, []) === true;
