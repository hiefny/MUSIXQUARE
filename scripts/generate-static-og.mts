...
interface StaticCardDefinition {
  ...
  readonly props: Omit<CardProps, 'wordmarkDataUrl'>;
}

export interface StaticOgInputs {
  readonly bold: Buffer;
  readonly extrabold: Buffer;
  readonly wasm: Buffer;
  readonly wordmarkRaw: string;
}

export interface RenderedStaticOgCard {
  readonly outFile: string;
...
export async function renderStaticOgCards({
  ...
  for (const { outFile, props } of CARDS) {
    ...
    renderedCards.push({ outFile, png, elapsedMs: performance.now() - t0 });
  }
...
async function main(): Promise<void> {
  ...
  const renderedCards = await renderStaticOgCards({ bold, extrabold, wasm, wordmarkRaw });
  for (const { outFile, png, elapsedMs } of renderedCards) {
    await writeFile(path.join(repoRoot, outFile), png);
    console.log(
      `  ${outFile.padEnd(28)}  ${elapsedMs.toFixed(0).padStart(4)} ms  ${(png.length / 1024).toFixed(1).padStart(6)} KB`,
    );
  }
...
if (invokedPath === import.meta.url) {
  main().catch((error: unknown) => {
    console.error('\n❌ Static OG generation failed:', error);
    process.exit(1);
  });
}
