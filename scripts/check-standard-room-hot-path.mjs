...
function sourceFile(fileName, source) {
  ...
  if (diagnostics.length > 0) {
    ...
    throw new Error(`${fileName} could not be parsed near offset ${first.start ?? 0}.`);
  }
  return parsed;
}

function nodeText(node, parsed) {
...
function callsNamed(root, names) {
  return collect(root, (node) => ts.isCallExpression(node) && names.has(callName(node)));
}

function localCallables(parsed) {
  const callables = new Map();
  for (const node of collect(parsed, (candidate) => ts.isFunctionDeclaration(candidate))) {
    if (node.name?.text && node.body) callables.set(node.name.text, node);
  }
  for (const node of collect(parsed, (candidate) => ts.isVariableDeclaration(candidate))) {
    if (
...
function importedNames(parsed, modulePath) {
  const names = new Set();
...
function reachableRemoteCalls(root, parsed, remoteNames, options = {}) {
  const callables = localCallables(parsed);
  const boundaries = [];
  const expand = (node, stack) => {
    for (const call of collect(node, (candidate) => ts.isCallExpression(candidate))) {
...
function taintedTurnIdentifiers(scope, parsed) {
...
function propertyValue(objectLiteral, propertyName, parsed) {
...
function findDefaultFetchMethod(parsed) {
  ...
  const method = exports[0].expression.properties.find(
...
function isInsideProBranch(node, parsed) {
  let child = node;
  let parent = node.parent;
  while (parent) {
...
function assertTurnAdmission(appWorkerSource, failures) {
  ...
  const options = guardCalls[0].arguments.at(-1);
  if (
    !options ||
  ...
  const serviceControlNames = importedNames(parsed, './service-maintenance.js');
...
function assertSignalingBoundary(signalingWorkerSource, failures) {
...
function assertInviteReturnsBeforeTurn(peerSource, failures) {
  ...
  if (
    ...
    !nodeText(declarations[0].initializer, parsed).includes('getStandardRoomTurnCredentials')
  ) {
  ...
  const branch = claimBranches[0].thenStatement;
  ...
  const transportStart = transportCalls[0].getStart(parsed);
  ...
  if (declarations[0].getStart(parsed) > claimBranches[0].getStart(parsed)) {
  ...
  if (backgroundSettles.length !== 1 || !ts.isVoidExpression(backgroundSettles[0].parent)) {
  ...
  const inviteReturns = directReturns.filter((node) => nodeText(node.expression, parsed) === 'id');
  ...
  if (
    ...
    !ts.isAwaitExpression(idDeclarations[0].initializer) ||
    !containsIdentifier(
      nodeText(idDeclarations[0].initializer.expression, parsed),
      'peerOpenRequest',
    )
  ) {
...
export function assertStandardRoomHotPath(sources) {
  const failures = [];
