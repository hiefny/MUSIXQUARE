...
export interface FilePlaybackProductHostMediaOwnerOptions {
  ...
  /**
   * Product-owned delivery decision. Omission preserves the established
   * peer-range route for isolated owner tests and custom embedders.
   */
  readonly selectBoundedTransport?: (connection: DataConnection) => 'peer-range' | 'r2-records';
  /** Resolve the current standard-room storage namespace only on an R2 route. */
  readonly resolveR2StorageRoomId?: () => string;
...
interface R2RecordOfferFields {
  readonly storageRoomId: string;
  readonly setId: string;
  readonly recordSize: number;
  readonly recordCount: number;
  readonly recordObjectIds: string;
...
export class FilePlaybackProductHostMediaOwner {
  ...
  readonly #hostRoom: FilePlaybackProductHostMediaRoomPort;
  readonly #publisher: FilePlaybackR2WholeBlobPublisher;
  readonly #selectBoundedTransport: NonNullable<
    FilePlaybackProductHostMediaOwnerOptions['selectBoundedTransport']
  >;
  readonly #resolveR2StorageRoomId: NonNullable<
    FilePlaybackProductHostMediaOwnerOptions['resolveR2StorageRoomId']
  >;
  ...
  constructor(options: FilePlaybackProductHostMediaOwnerOptions) {
    ...
    if (
      ...
      (input.selectBoundedTransport !== undefined &&
        typeof input.selectBoundedTransport !== 'function') ||
      (input.resolveR2StorageRoomId !== undefined &&
        typeof input.resolveR2StorageRoomId !== 'function')
    ) {
    ...
    this.#selectBoundedTransport =
      (input.selectBoundedTransport as
        | NonNullable<FilePlaybackProductHostMediaOwnerOptions['selectBoundedTransport']>
        | undefined) ?? (() => 'peer-range');
    this.#resolveR2StorageRoomId =
      (input.resolveR2StorageRoomId as
        | NonNullable<FilePlaybackProductHostMediaOwnerOptions['resolveR2StorageRoomId']>
        | undefined) ??
      (() => {
        throw new Error('V2 R2 record storage room resolver is unavailable');
      });
    this.#sendRequired =
  ...
  activatePrepared(
    ...
    const peerRangeOfferPlan =
      publication?.backend === 'bounded-stream' && isAnyPeerRangeOffer(record.offer)
  ...
  async #publishWarmSourceOffer(
    ...
    const useR2Records = this.#usesR2RecordTransport();
    const peerRangeOfferPlan = useR2Records
      ? null
      : this.#createPeerRangeOfferPlan(authority.backend, asset);
    ...
    let r2RecordFields: Readonly<R2RecordOfferFields> | null = null;
    if (useR2Records) {
      if (!(exactSource instanceof Blob)) {
        await exactSource.close().catch(() => undefined);
        throw new Error('Host warm R2 record publication requires its exact Blob source');
      }
      r2RecordFields = await this.#publishR2RecordOfferFields(
        exactSource,
        asset,
        controller.signal,
      );
    } else if (isEncodedSource(exactSource)) {
      await exactSource.close();
    }
    this.#assertWarmSourceOfferTask(authority, sourceLease, epoch, controller);
    const expiresAtRoomTimeMs =
      r2RecordFields?.expiresAtRoomTimeMs ??
    ...
    try {
      ...
      if (useR2Records) {
        if (!r2RecordFields) throw new Error('Host warm R2 record offer plan is incomplete');
        offer = createR2RecordFileMediaSourceOfferV2({
          ...base,
          ...r2RecordFields,
        });
      } else {
        if (!peerRangeOfferPlan || !handleId) {
          throw new Error('Host warm peer-range offer plan is incomplete');
        }
        const peerBase = { ...base, handleId } as const;
  ...
  async #publish(
    ...
    const useR2Records = publication.backend === 'bounded-stream' && this.#usesR2RecordTransport();
    const peerRangeOfferPlan =
      publication.backend === 'bounded-stream' && !useR2Records
        ? this.#createPeerRangeOfferPlan(publication.backend, publication.asset)
        : null;
    ...
    let r2RecordFields: Readonly<R2RecordOfferFields> | null = null;
    if (publication.backend === 'bounded-stream' && !useR2Records) {
      ...
      if (!(source instanceof Blob)) {
        ...
        throw new Error('Host R2 publication requires its exact Blob source');
      }
      this.#assertPublicationAuthority(publication, epoch);
      if (useR2Records) {
        r2RecordFields = await this.#publishR2RecordOfferFields(source, publication.asset, signal);
      } else {
    ...
    try {
      ...
      let offer: Readonly<FileMediaSourceOfferV2>;
      if (useR2Records) {
        if (!r2RecordFields) throw new Error('Host R2 record offer plan is incomplete');
        offer = createR2RecordFileMediaSourceOfferV2({
          ...base,
          prepareRevision,
          ...r2RecordFields,
        });
      } else if (publication.backend === 'bounded-stream') {
        if (
          handleId === null ||
  ...
  async #publishPrepared(
    ...
    const useR2Records = prepared.backend === 'bounded-stream' && this.#usesR2RecordTransport();
    const peerRangeOfferPlan =
    ...
    let r2RecordFields: Readonly<R2RecordOfferFields> | null = null;
    if (prepared.backend === 'bounded-stream' && !useR2Records) {
      ...
      peerRangeExpiresAtRoomTimeMs =
        this.#nowRoomTimeMs() + FILE_PLAYBACK_PRODUCT_OFFER_LIFETIME_MS;
    } else if (useR2Records) {
      if (!(exactSource instanceof Blob)) {
        await exactSource.close().catch(() => undefined);
        throw new Error('Host prepared R2 record publication requires its exact Blob source');
      }
      this.#assertPreparedCandidateAuthority(prepared, epoch);
      this.#fenceSameLeaseWarmSourceForPreparedRepair(prepared);
      this.#assertPreparedCandidateAuthority(prepared, epoch);
      r2RecordFields = await this.#publishR2RecordOfferFields(exactSource, prepared.asset, signal);
    } else {
      if (!(exactSource instanceof Blob)) {
        await exactSource.close().catch(() => undefined);
    ...
    try {
      ...
      let offer: Readonly<FileMediaSourceOfferV2>;
      if (useR2Records) {
        if (!r2RecordFields) throw new Error('Host prepared R2 record offer plan is incomplete');
        offer = createR2RecordFileMediaSourceOfferV2({
          ...base,
          prepareRevision,
          ...r2RecordFields,
        });
      } else if (prepared.backend === 'bounded-stream') {
        if (
          handleId === null ||
          peerRangeExpiresAtRoomTimeMs === null ||
          peerRangeOfferPlan === null
  ...
  #canTransferWarmSourceToPrepared(
    ...
    if (!isAnyPeerRangeOffer(warm.offer)) return false;
  ...
  #transferWarmSourceToPrepared(
    ...
    if (!isAnyPeerRangeOffer(warm.offer)) {
      throw new Error('Host warm R2 source offer cannot transfer peer-range authority');
    }
    this.#assertSamePeerRangeOfferPlan(
  ...
  async #resolvePeerSource(
    ...
    const peerRangeOfferPlan = this.#requirePeerRangeOfferPlan(warm.offer, warm.peerRangeOfferPlan);
  ...
  #retireWarmSourceOffer(
    ...
    if (isAnyPeerRangeOffer(record.offer) && record.handleId !== null) {
      try {
        this.#responder.revokeHandle(
          this.#context.connectionToken,
          record.handleId,
  ...
  async #publishR2RecordOfferFields(
    source: Blob,
    asset: Readonly<HostPeerPlaybackAssetPublication>,
    signal: AbortSignal,
  ): Promise<Readonly<R2RecordOfferFields>> {
    const storageRoomId = this.#resolveR2StorageRoomId();
    if (!/^[1-9]\d{5}$/u.test(storageRoomId)) {
      throw new Error('V2 R2 record delivery requires an exact standard room code');
    }
    const published = await awaitWhileOwned(
      this.#publisher.publishRecordSet(
        {
          queueItemId: asset.binding.queueItemId,
          sourceIdentity: asset.binding.sourceIdentity,
          transferSessionId: asset.binding.transferSessionId,
        ...
        {
          storageRoomId,
          applicationSessionId: this.#context.sessionId,
        },
      ),
      signal,
    );
    ...
    if (
      published.encodedSize !== asset.encodedSize ||
      published.sourceIdentity !== asset.binding.sourceIdentity ||
      published.transferSessionId !== asset.binding.transferSessionId ||
      published.applicationSessionId !== this.#context.sessionId ||
      published.storageRoomId !== storageRoomId ||
      published.cryptoSecretDescriptor.objectId !== published.setId ||
      published.cryptoSecretDescriptor.plaintextSize !== asset.encodedSize ||
      published.records.length !== published.recordCount
    ) {
      throw new Error('Host R2 record publication changed its exact binding');
    }
    return freezeCanonical({
      storageRoomId,
      setId: published.setId,
      recordSize: published.recordSize,
      recordCount: published.recordCount,
      recordObjectIds: published.records.map((record) => record.objectId).join(','),
      keyB64: published.cryptoSecretDescriptor.keyB64,
      noncePrefixB64: published.cryptoSecretDescriptor.noncePrefixB64,
      expiresAtRoomTimeMs:
        this.#nowRoomTimeMs() + Math.max(0, published.expiresAtEpochMs - epochNow),
    });
  }

  #usesR2RecordTransport(): boolean {
    const selected = this.#selectBoundedTransport(this.#context.connection);
    if (selected !== 'peer-range' && selected !== 'r2-records') {
      throw new Error('Host bounded delivery selector returned an invalid transport');
    }
    return selected === 'r2-records';
  }
  ...
  #assertWarmSourceOfferRecord(record: WarmSourceOfferRecord): void {
    ...
    if (isAnyPeerRangeOffer(record.offer)) {
      if (record.offer.handleId !== record.handleId || !record.peerRangeOfferPlan) {
    ...
    } else if (record.handleId !== null || record.peerRangeOfferPlan !== null) {
      throw new Error('Host warm R2 source offer changed its exact plan');
    }
  }
