...
export class FilePlaybackRunAuthority {
  ...
  /**
   * Consumes an exact active late-join baseline that the host stopped before
   * this guest could commit renderer readiness.
   */
  commitUnpreparedBaselineStop(
    token: object,
    lease: FilePlaybackRunLease,
    ...
    this.#mutate(token, () => {
      const candidate = this.#requireRecord(lease);
      const expected = readExpectedState(expectedCurrent);
      const next = readExpectedState(stopped);
      if (
        !expected ||
        !next ||
        ...
        !sameState(candidate.binding, expected) ||
        next.queueItemId !== expected.queueItemId ||
        next.runId !== expected.runId ||
        next.revision !== expected.revision + 1
      ) {
        throw new Error('Only the exact unprepared active baseline can commit STOP');
      }
      this.#retireRecord(candidate);
      this.#revisionWatermark = next.revision;
      this.#bootstrapKind = 'stopped';
    });
  }
