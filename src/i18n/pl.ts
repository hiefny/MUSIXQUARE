/**
 * MUSIXQUARE - Polish Translations
 *
 * Polished premium translations. Aligned with ko.ts keys.
 */

import type { I18nKey } from './ko.ts';

const pl: Record<I18nKey, string> = {
  // ─── Common ──────────────────────────────────────────────────────
  'common.ok': 'OK',
  'common.cancel': 'Anuluj',
  'common.close': 'Zamknij',
  'common.reset': 'Resetuj',
  'common.refresh': 'Odśwież',
  'common.later': 'Później',
  'common.next': 'Dalej',
  'common.start': 'Rozpocznij',
  'common.info': 'Info',
  'common.stay': 'Zostań',
  'common.leave': 'Wyjdź',
  'common.granted': 'Przyznane',
  'common.revoked': 'Odwołane',
  'common.stop': 'Zatrzymaj',
  'common.pause': 'Pauza',
  'common.message': 'Wiadomość',
  'common.mix': 'Mix',
  'common.wait': 'Chwileczkę...',
  'common.guest': 'Gość',
  'common.help': 'Pomoc',
  'common.increase': 'Zwiększ',
  'common.decrease': 'Zmniejsz',
  'common.done': 'Ukończono',
  'common.sync': 'Synchronizuj',
  'common.on': 'Wł.',
  'common.off': 'Wył.',
  'common.previous': 'Poprzedni',
  'common.next_desc': 'Następny',
  'common.volume_percent': 'Głośność: {{val}}%',
  'common.muted': 'Wyciszono',
  'common.original': 'Oryginał',
  'common.left': 'Lewy',
  'common.right': 'Prawy',
  'common.woofer': 'Woofer',
  'common.connected': 'Połączono',
  'common.disconnected': 'Rozłączono',
  'common.revoke': 'Odbierz uprawnienia administratora',
  'common.grant': 'Nadaj uprawnienia administratora',
  'common.system': 'System',
  'common.unknown': 'Nieznany',
  'common.youtube_video': 'Wideo z YouTube',
  'common.peer': 'Peer',

  // ─── Navigation ──────────────────────────────────────────────────
  'nav.home': 'Main',
  'nav.playlist': 'Playlist',
  'nav.settings': 'Ustawienia',
  'nav.help': 'Pomoc',
  'nav.connect': 'Połączenie',
  'nav.go_home': 'Przejdź do strony głównej',

  // ─── Player ──────────────────────────────────────────────────────
  'player.no_media': 'Brak odtwarzanych multimediów',
  'player.select_file_hint': 'Wybierz plik lub sprawdź playlistę',
  'player.play_pause': 'Odtwórz/Wstrzymaj',
  'player.visualizer': 'Wizualizator audio',
  'player.seek': 'Regulacja pozycji odtwarzania',
  'player.volume': 'Regulacja głośności',
  'player.toggle_mute': 'Wycisz / Wyłącz wyciszenie',
  'player.fullscreen': 'Tryb pełnoekranowy',
  'player.prev_track': 'Poprzedni utwór',
  'player.next_track': 'Następny utwór',
  'player.repeat_mode': 'Zmień tryb powtarzania',
  'player.shuffle_mode': 'Zmień tryb losowy',
  'player.play_media': 'Odtwórz utwór',
  'player.play_media_action': 'Odtwórz utwór',
  'player.add_media': 'Dodaj utwór',
  'player.play_start': 'Dodaj do kolejki',
  'player.select_media': 'Wybierz utwór do odtworzenia',
  'player.play_together': 'Synchronizuj odtwarzanie',
  'player.play_speakers': 'Użyj jako głośnika',
  'player.auto_sync_label': 'Auto synchronizacja (ms)',
  'player.manual_sync_label': 'Ręczna synchronizacja (ms)',

  // ─── Playlist ────────────────────────────────────────────────────
  'playlist.toggle': 'Pokaż/ukryj playlistę',
  'playlist.repeat_all': 'Powtarzanie wszystkich utworów włączone',
  'playlist.repeat_one': 'Powtarzanie jednego utworu włączone',
  'playlist.repeat_off': 'Powtarzanie wyłączone',
  'playlist.shuffle_on': 'Odtwarzanie losowe włączone',
  'playlist.shuffle_off': 'Odtwarzanie losowe wyłączone',
  'playlist.empty_hint': 'Dodaj utwory do playlisty.',
  'playlist.empty_hint_guest': 'Utwory pojawią się, gdy host je doda.',
  'playlist.loading_info': 'Ładowanie informacji o utworze...',
  'playlist.sub_load_failed': 'Nie udało się załadować informacji o wideo',
  'playlist.remove_title': 'Usuń z playlisty',
  'playlist.remove_message': 'Czy chcesz usunąć ten utwór z playlisty?',
  'playlist.remove_yes': 'Usuń',
  'playlist.remove_no': 'Nie',
  'playlist.video_fallback': 'Ładowanie...',
  'playlist.track_fallback': 'Utwór {{idx}}',
  'playlist.deferred_load_hint': 'Uruchom odtwarzanie, aby załadować informacje o wszystkich utworach',

  // ─── Role ────────────────────────────────────────────────────────
  'role.center': 'Głośnik centralny',
  'role.left': 'Głośnik lewy',
  'role.right': 'Głośnik prawy',
  'role.subwoofer': 'Subwoofer',
  'role.center_placement': 'Umieść urządzenie na środku',
  'role.left_placement': 'Umieść urządzenie po lewej stronie',
  'role.right_placement': 'Umieść urządzenie po prawej stronie',

  // ─── Settings ────────────────────────────────────────────────────
  'settings.theme': 'Tema',
  'settings.dark': 'Ciemny',
  'settings.light': 'Jasny',
  'settings.visualizer': 'Wizualizator',
  'settings.viz_circular': 'Okrągły',
  'settings.viz_spectrum': 'Widmo',
  'settings.language': 'Język · Language',
  'settings.language_select': 'Wybierz',
  'settings.language_system': 'System',
  'settings.language_dialog_title': 'Wybierz język',
  'settings.language_select_aria': 'Otwórz menu wyboru języka',
  'settings.language_system_aria': 'Użyj języka systemu',
  'settings.advanced_audio': 'Zaawansowane audio',
  'settings.spatial_audio': 'Dźwięk przestrzenny',
  'settings.reverb_title': 'Pogłos',
  'settings.host_ctrl': 'Kontrola Host',
  'settings.self_ctrl': 'Kontrola lokalna',
  'settings.rvb_studio': 'Studio',
  'settings.rvb_arena': 'Hala koncertowa',
  'settings.rvb_advanced': 'Zaawansowane',
  'settings.rvb_off': 'Wył.',
  'settings.reverb_mix': 'Mix pogłosu',
  'settings.reverb_decay': 'Czas pogłosu',
  'settings.reverb_predelay': 'Pre-delay pogłosu',
  'settings.reverb_lowcut': 'Low cut pogłosu',
  'settings.reverb_highcut': 'High cut pogłosu',
  'settings.decay_time': 'Czas pogłosu',
  'settings.predelay': 'Pre-delay',
  'settings.eq_title': 'Korektor',
  'settings.eq_bright': 'Klarowność',
  'settings.eq_warm': 'Ciepło',
  'settings.eq_advanced': 'Zaawansowane',
  'settings.eq_off': 'Wył.',
  'settings.lowpass': 'Filtr dolnoprzepustowy',
  'settings.highpass': 'Filtr górnoprzepustowy',
  'settings.surround_title': 'Wirtualny surround',
  'settings.bass_title': 'Wirtualny bas',
  'settings.exciter_title': 'Wirtualny sopran',
  'settings.subwoofer_adjust': 'Crossover subwoofera',
  'settings.subwoofer_cutoff': 'Częstotliwość crossovera',
  'settings.default_0pct': 'Domyślnie: 0%',
  'settings.default_01s': 'Domyślnie: 0.1s',
  'settings.default_20khz': 'Domyślnie: 20.0kHz',
  'settings.default_20hz': 'Domyślnie: 20Hz',
  'settings.default_5s': 'Domyślnie: 5.0s',
  'settings.invite_code': 'Kod zaproszenia',
  'settings.leave_session': 'Wyjdź z sesji',
  'connect.rename_device': 'Zmień nazwę tego urządzenia',
  'connect.rename_title': 'Zmień nazwę urządzenia',
  'connect.rename_message': 'Wybierz pseudonim widoczny dla innych.',
  'connect.rename_placeholder': 'Nowy pseudonim (maks. 20 znaków)',
  'connect.rename_current': 'Obecny pseudonim',
  'connect.rename_empty': 'Pseudonim nie może być pusty.',
  'connect.rename_reserved': 'Ten pseudonim jest zarezerwowany.',
  'connect.rename_profanity': 'Wykryto nieodpowiedni pseudonim.',
  'connect.rename_duplicate': 'Ten pseudonim jest już używany.',
  'settings.devices_title': 'Podłączone urządzenia',
  'settings.invite_share': 'Zaproszenia i udostępnianie',
  'settings.subtab.general': 'Ogólne',
  'settings.subtab.audio': 'Audio',
  'settings.subtab.connect': 'Połączenie',
  'settings.subtab.help': 'Pomoc',

  // ─── Connect ───────────────────────────────────────────────────
  'connect.qr_title': 'Zaproś przez kod QR lub link',
  'connect.max_devices': 'Pojemność pokoju',
  'connect.device_list': '{{count}} połączonych urządzeń',
  'connect.device_list_one': '{{count}} połączone urządzenie',
  'connect.no_session': 'Najpierw utwórz pokój',
  'connect.generating_qr': 'Generowanie kodu QR...',
  'connect.cannot_reduce': 'Nie można zmniejszyć pojemności: {{count}} urządzeń jest już połączonych',
  'connect.kick_title': 'Odłącz urządzenie',
  'connect.kick_message': 'Czy chcesz usunąć to urządzenie z pokoju?',
  'connect.kick_yes': 'Usuń',
  'connect.kick_no': 'Nie',
  'connect.kicked_title': 'Rozłączono',
  'connect.kicked_message': 'Zostałeś usunięty z pokoju przez hosta.',
  'connect.copy_invite_link': 'Skopiuj link zaproszenia',
  'connect.link_copied': 'Link zaproszenia skopiowany',
  'connect.room_password_title': 'Wymagaj hasła przy wejściu do pokoju',
  'connect.room_password_set': 'Ustaw',
  'connect.room_password_toggle_aria': 'Przełącz wymaganie hasła do pokoju',
  'connect.room_password_input_aria': 'Wpisz 8-cyfrowe hasło do pokoju',
  'connect.room_password_refresh_aria': 'Wygeneruj nowe hasło do pokoju',
  'connect.room_password_off': '- - - - - - - -',
  'connect.room_password_pending': 'Oczekiwanie na 8-cyfrowe hasło',
  'connect.room_password_on': 'Hasło aktywne',
  'connect.room_password_invalid': 'Wpisz 8-cyfrowe hasło',
  'connect.room_password_enabled': 'Wejście do tego pokoju wymaga teraz hasła',
  'connect.room_password_disabled': 'Wejście do tego pokoju nie wymaga już hasła',

  // ─── Setup ───────────────────────────────────────────────────────
  'setup.host_button': 'Utwórz pokój',
  'setup.guest_button': 'Dołącz do pokoju',
  'setup.demo_button': 'Wypróbuj aplikację (Demo)',
  'demo.prompt_title': 'Odkryj MUSIXQUARE',
  'demo.prompt_message': 'Witaj! Chcesz odbyć krótki przewodnik po naszych funkcjach?',
  'demo.prompt_cta': 'Rozpocznij przewodnik',
  'demo.prompt_later': 'Później',
  'demo.try_later_toast': 'Możesz rozpocząć przewodnik w dowolnej chwili z karty Pomoc',
  'demo.track_info': 'Informacje o utworze',
  'demo.exit': 'Zakończ przewodnik',
  'demo.credit': 'Utwór demo',
  'demo.role_title': 'Ustaw rolę tego urządzenia',
  'demo.effects_title': 'Wypróbuj efekty dźwiękowe',
  'demo.reverb': 'Pogłos',
  'demo.bass_boost': 'Wzmocnienie basów',
  'demo.treble_boost': 'Wzmocnienie sopranów',
  'demo.surround': 'Surround',
  'demo.role_bass_short': 'Basy',
  'demo.role_center_short': 'Centrum',
  'demo.step_nav_label': 'Etapy przewodnika',
  'demo.step_connect': '1. Podłącz urządzenia',
  'demo.step_role': '2. Ustaw rolę',
  'demo.step_effects': '3. Efekty dźwiękowe',
  'demo.step_finish': '4. Zakończ przewodnik',
  'demo.session_title': 'Znajdujesz się w pokoju {{code}}',
  'demo.session_subtitle': 'Możesz zaprosić inne urządzenia w dowolnym momencie',
  'demo.session_body_connected':
    'Obecnie połączonych jest {{count}} urządzeń.\nPrzypisz rolę do każdego urządzenia.',
  'demo.session_body_alone': 'Obecnie odtwarzasz muzykę sam.\nZaproś teraz inne urządzenia.',
  'demo.effects_step_title': 'Kształtuj bogatszy dźwięk.',
  'demo.effects_step_subtitle': 'Szczegółowe ustawienia możesz dostosować w dowolnej chwili w Ustawieniach.',
  'demo.finish_step_title': 'Przewodnik wprowadzający dobiegł końca.',
  'demo.finish_step_subtitle': 'Wróć do aplikacji i zacznij korzystać ze wszystkich funkcji!',
  'demo.track_list_label': 'Lista utworów demo',
  'demo.host_only_exit': 'Tylko host pokoju może wykonać tę operację.',
  'demo.external_playback_stopped':
    'Przewodnik demo zatrzymał bieżące odtwarzanie YouTube lub dźwięku systemowego.',
  'setup.how_to_connect': 'Jak się połączyć',
  'setup.set_role': 'Ustaw rolę tego urządzenia',
  'setup.what_sound': 'W co powinno grać to urządzenie?',
  'setup.change_later': 'Będziesz mógł to zmienić w dowolnym momencie w Ustawieniach.',
  'setup.welcome_title': 'Witamy',
  'setup.join_session_subtitle': 'Zaraz dołączysz do pokoju {{code}}',
  'setup.select_role': 'Wybierz rolę',
  'setup.select_role_alt': 'Wybierz rolę',
  'setup.hello_select_role': 'Cześć! Proszę wybrać swoją rolę.',
  'setup.enter_code': 'Wpisz 6-cyfrowy kod połączenia',
  'setup.enter_host_code': 'Wpisz 6-cyfrowy kod podany przez hosta',
  'setup.enter_host_code_alt': 'Wpisz kod hosta.',
  'setup.connect_devices': 'Now podłącz inne urządzenia.',
  'setup.enter_code_connect': 'Wpisz ten kod na innych urządzeniach',
  'setup.last_step': 'Zaczynamy?',
  'setup.joining': 'Dołączanie...',
  'setup.six_digit_enter': 'Wpisz 6-cyfrowy kod',

  // ─── Help ────────────────────────────────────────────────────────
  'help.need_help': 'Potrzebujesz pomocy?',
  'help.install_app': 'Zainstaluj aplikację webową',
  'help.local_recommended': 'Zalecana sieć lokalna',
  'help.connect_all_wifi': 'Połączenie jest najbardziej stabilne, gdy wszystkie urządzenia korzystają z tej samej sieci Wi-Fi',
  'help.local_file': 'Wybierz plik lokalny',
  'help.load_local': 'Załaduj plik lokalny',
  'help.youtube_compat': 'YouTube (Tryb zgodności)',
  'help.youtube_no_effects': 'W trybie YouTube zaawansowane efekty dźwiękowe i zarządzanie rolami głośników są niedostępne.',
  'help.media_synced_detail': 'Wybrane utwory odtwarzają się w idealnej synchronizacji na wszystkich połączonych urządzeniach.',
  'help.wifi_hotspot': 'Możesz również użyć mobilnego punktu dostępowego (Hotspot), aby połączyć wszystkie urządzenia do jednej sieci',

  // ─── Chat ────────────────────────────────────────────────────────
  'chat.title': 'Czat',
  'chat.close': 'Zamknij czat',
  'chat.open': 'Otwórz czat',
  'chat.placeholder': 'Napisz wiadomość lub wpisz „/”, aby zobaczyć polecenia...',
  'chat.input_label': 'Wiadomość na czacie',
  'chat.send': 'Wyślij',
  'chat.start': 'Brak wiadomości. Napisz coś, aby rozpocząć czat!',

  // ─── Chat Commands ──────────────────────────────────────────────
  'chat.muted_placeholder': 'Nie możesz wysyłać wiadomości na tym czacie',
  'chat.cmd_frozen': 'Czat został zamrożony',
  'chat.cmd_unfrozen': 'Zamrożenie czatu zostało cofnięte',
  'chat.cmd_frozen_blocked': 'Czat jest obecnie zamrożony',
  'chat.cmd_muted': '{{name}} został wyciszony',
  'chat.cmd_unmuted': 'Wyciszenie {{name}} zostało cofnięte',
  'chat.cmd_clear': 'Historia czatu została wyczyszczona',
  'chat.cmd_filter_on': 'Filtr słów niecenzuralnych włączony',
  'chat.cmd_filter_off': 'Filtr słów niecenzuralnych wyłączony',
  'chat.cmd_slowmode_on': 'Slow mode włączony: możesz wysłać wiadomość co {{sec}} sekund',
  'chat.cmd_slowmode_off': 'Slow mode wyłączony',
  'chat.cmd_slowmode_wait': 'Poczekaj {{sec}} sekund przed wysłaniem kolejnej wiadomości',
  'chat.cmd_notice_prefix': 'Ogłoszenie',
  'chat.system_sender': 'System',
  'chat.decode_skip_notice': 'Niektóre urządzenia nie mogły odtworzyć tego utworu. Przejście do kolejnego utworu...',
  'chat.system_audio_started_notice':
    'Rozpoczęto udostępnianie dźwięku systemowego. Ponieważ opopóźnienie między hostem a gośćmi jest nieuniknione, zalecamy hostowi maksymalne zmniejszenie głośności swojego urządzenia.',
  'chat.system_audio_stopped_notice':
    'Udostępnianie dźwięku systemowego zostało zakończone. Host może ponownie podgłośnić swoje urządzenie, aby powrócić do roli głośnika.',
  'chat.remote_guest_detected_notice':
    'Dołączył gość zdalny. Jakość połączenia może się różnić w zależności od stabilności sieci.',
  'chat.remote_upload_limited_notice':
    'Osiągnięto limit przesyłania plików.\nGoście zdalni mogą spróbować ponownie później.',
  'chat.remote_upload_failed_notice':
    'Nie udało się przesłać pliku zdalnego.\nZdalni goście mogą spróbować ponownie za chwilę.',
  'chat.pinned_close': 'Zamknij ogłoszenie',
  'chat.cmd_whisper_from': 'Wiadomość prywatna od {{name}}',
  'chat.cmd_whisper_to': 'Wiadomość prywatna do {{name}}',
  'chat.cmd_target_not_found': 'Odbiorca nie znaleziony: {{target}}',
  'chat.cmd_no_permission': 'Nie masz uprawnień do korzystania z tego polecenia',
  'chat.cmd_nick_changed': 'Pseudonim został zmieniony na {{name}}',
  'chat.cmd_nick_too_long': 'Pseudonim może mieć maksymalnie 20 znaków',
  'chat.cmd_already_op': '{{name}} jest już administratorem',
  'chat.cmd_not_op': '{{name}} nie jest administratorem',
  'chat.cmd_help_title': 'Dostępne polecenia',
  'chat.cmd_help_target_hint': '* [urządzenie] to identyfikator urządzenia (np. #1)',
  'chat.cmd_unknown': 'Nieznane polecenie: /{{cmd}}',
  'chat.cmd_usage': 'Użycie: {{usage}}',
  'chat.cmd_users_title': 'Lista użytkowników',
  'chat.cmd_users_me': 'Ty',

  // ─── Command usage & description (i18n) ────────────────────────
  'chat.cmd_u_help': '/help',
  'chat.cmd_u_users': '/users',
  'chat.cmd_u_clear': '/clear',
  'chat.cmd_u_filter': '/filter [on | off]',
  'chat.cmd_u_freeze': '/freeze [on | off]',
  'chat.cmd_u_slowmode': '/slowmode [sekundy]',
  'chat.cmd_u_w': '/w [urządzenie] [wiadomość]',
  'chat.cmd_u_whisper': '/whisper [urządzenie] [wiadomość]',
  'chat.cmd_u_notice': '/notice [wiadomość]',
  'chat.cmd_u_nick': '/nick [nowy pseudonim]',
  'chat.cmd_u_kick': '/kick [urządzenie]',
  'chat.cmd_u_op': '/op [urządzenie]',
  'chat.cmd_u_deop': '/deop [urządzenie]',
  'chat.cmd_u_mute': '/mute [urządzenie]',
  'chat.cmd_u_unmute': '/unmute [urządzenie]',
  'chat.cmd_d_help': 'Wyświetla listę dostępnych poleceń',
  'chat.cmd_d_users': 'Pokazuje listę połączonych użytkowników',
  'chat.cmd_d_clear': 'Usuwa historię czatu',
  'chat.cmd_d_filter': 'Włącza/wyłącza filtr słów niecenzuralnych',
  'chat.cmd_d_freeze': 'Zamraża/odmraża czat grupowy',
  'chat.cmd_d_slowmode': 'Ustala odstęp czasowy między wiadomościami (slow mode)',
  'chat.cmd_d_w': 'Wysyła prywatną wiadomość do urządzenia',
  'chat.cmd_d_notice': 'Wysyła ogólne ogłoszenie do wszystkich',
  'chat.cmd_d_nick': 'Zmienia pseudonim Twojego urządzenia',
  'chat.cmd_d_kick': 'Odłącza wskazane urządzenie z pokoju',
  'chat.cmd_d_op': 'Nadaje uprawnienia administratora wskazanemu urządzeniu',
  'chat.cmd_d_deop': 'Odbiera uprawnienia administratora wskazanemu urządzeniu',
  'chat.cmd_d_mute': 'Blokuje możliwość pisania na czacie wskazanemu urządzeniu',
  'chat.cmd_d_unmute': 'Cofa blokadę pisania na czacie wskazanemu urządzeniu',
  'chat.cmd_u_debug': '/debug',
  'chat.cmd_d_debug': 'Wyświetla systemowe informacje debugowania',
  'chat.debug_copied': 'Informacje debugowania zostały skopiowane do schowka.\nJeśli napotkasz błędy, prześlij je programiście.',
  'chat.seek_to': 'Przejdź do {{time}}',

  // ─── YouTube ─────────────────────────────────────────────────────
  'youtube.enter_link': 'Wpisz link z YouTube',
  'youtube.enter_source': 'Szukaj utworu lub wpisz link z YouTube',
  'youtube.enter_link_prompt': 'Wpisz hasło wyszukiwania lub link z YouTube',
  'youtube.enter_source_placeholder': 'Wpisz hasło lub link (https://youtube.com/...)',
  'youtube.watch_together': 'Wspólne oglądanie YouTube',
  'youtube.ready': 'YouTube jest gotowy!\nNaciśnij Odtwórz, aby rozpocząć.',
  'youtube.thumbnail': 'Miniatura podglądu wideo z YouTube',
  'youtube.load_fail': 'Nie udało się załadować API YouTube.\nSprawdź połączenie z internetem!',
  'youtube.video_unavailable': 'Wideo niedostępne. Przejście do kolejnego utworu...',
  'youtube.load_timeout': 'Przekroczono limit czasu ładowania YouTube.\nSpróbuj ponownie.',
  'youtube.invalid_link': 'Niepoprawny link z YouTube',
  'youtube.effects_disabled': 'W trybie wspólnego oglądania YouTube,\nzaawansowane efekty dźwiękowe są wyłączone.',
  'youtube.enter_link_placeholder': 'Wklej link do wideo lub playlisty',
  'youtube.enter_source_toast': 'Wpisz hasło wyszukiwania lub link z YouTube',
  'youtube.fetching_info': 'Pobieranie informacji o wideo...',
  'youtube.fetch_failed': 'Nie udało się pobrać informacji o wideo',
  'youtube.enter_link_toast': 'Wklej link z YouTube',
  'youtube.open_link': 'Otwórz link z YouTube',
  'youtube.tap_to_play': 'Dotknij, aby odtworzyć',
  'youtube.added_to_playlist': 'Dodano do playlisty',
  'youtube.loading_large_playlist': 'Wczytywanie dużej playlisty...',
  'youtube.indexing_playlist': 'Analiza playlisty w toku...',
  'youtube.search_button': 'Szukaj',
  'youtube.search_prompt': 'Naciśnij Enter lub Szukaj, aby znaleźć wideo',
  'youtube.searching': 'Wyszukiwanie w YouTube...',
  'youtube.search_no_results': 'Brak wyników wyszukiwania',
  'youtube.search_failed': 'Wyszukiwanie w YouTube jest obecnie niedostępne',
  'youtube.search_selected': 'Wybrane wideo gotowe do dodania do kolejki',
  'youtube.search_results': 'Wyniki wyszukiwania w YouTube',
  'youtube.live_sync_warning': 'Precyzyjna synchronizacja transmisji na żywo może być niestabilna.',

  // ─── Network ─────────────────────────────────────────────────────
  'network.disconnected': 'Połączenie przerwane',
  'network.cant_join': 'Nie znaleziono pokoju.\nSprawdź połączenie sieciowe.',
  'network.session_full': 'Pokój jest pełny',
  'network.session_full_detail':
    'Osiągnięto limit połączonych urządzeń.\nHost może zwiększyć ten limit w zakładce „Połączenie”.',
  'network.op_granted': 'Nadano Ci uprawnienia administratora.',
  'network.op_revoked': 'Odebrano Ci uprawnienia administratora.',

  // ─── Toast (dynamic) ────────────────────────────────────────────
  'toast.device_connected': '{{name}} połączył się',
  'toast.device_disconnected': '{{name}} rozłączył się',
  'toast.device_conn_error': 'Błąd połączenia z {{name}}',
  'toast.device_kicked': '{{name}} został usunięty z pokoju',
  'toast.remote_decode_device_wait':
    'Dekodowanie utworu nie powiodło się na jednym z urządzeń. Odtwarzanie jest kontynuowane na pozostałych.',
  'toast.invite_code': 'Kod zaproszenia: {{code}}',
  'toast.invite_code_info': 'Urządzenia: {{count}} | Kod: {{code}}',
  'toast.added_tracks': 'Dodano {{count}} utworów',
  'toast.preparing': 'Przygotowanie: {{name}}',
  'toast.file_save_error': 'Błąd zapisu pliku: {{name}}',
  'toast.distortion_warn': 'Dźwięk może ulec zniekształceniu',
  'toast.decoding_audio': 'Dekodowanie audio w toku...',
  'toast.preparing_next': 'Przygotowanie kolejnego utworu... ({{name}})',
  'toast.preparing_next_pct': 'Przygotowanie kolejnego utworu... {{pct}}%',
  'toast.op_status': 'Uprawnienia dla {{label}} zostały {{status}}',
  'toast.file_read_error': 'Błąd odczytu pliku: {{name}}',
  'toast.host_ended_connection': 'Host zamknął połączenie',

  // ─── Toast (static) ─────────────────────────────────────────────
  'toast.copy_failed': 'Kopiowanie nie powiodło się',
  'toast.email_copied': 'Adres email skopiowany do schowka!',
  'toast.host_only': 'Tylko host pokoju może wykonać tę operację.',
  'toast.host_only_media': 'Tylko host pokoju może dodawać utwory.',
  'toast.host_only_youtube': 'Tylko host pokoju może dodawać linki z YouTube.',
  'toast.host_only_control': 'Tylko host pokoju może kontrolować odtwarzanie.',
  'toast.host_setting_required': 'Tylko host pokoju może zmienić to ustawienie',
  'toast.host_only_file': 'Tylko host pokoju może dodawać pliki.',
  'toast.no_tracks': 'Brak utworów do odtworzenia',
  'toast.no_invite_code': 'Kod zaproszenia nie został jeszcze wygenerowany',
  'toast.invite_code_settings': 'Możesz udostępnić link zaproszenia z zakładki Połączenie.',
  'toast.click_copy_invite': 'Kliknij, aby skopiować kod zaproszenia',
  'toast.cant_select_file': 'Nie można wybrać pliku',
  'toast.video_excluded': 'Pliki wideo zostały wykluczone.',
  'toast.video_only_rejected': 'Dodawanie plików wideo jest niedozwolone.',
  'dialog.large_room_file.title': 'Udostępniaj pliki w dużym pokoju',
  'dialog.large_room_file.message':
    'Lokalne udostępnianie plików działa stabilnie przy maksymalnie 8 gościach w tej samej sieci lokalnej.\n\nCzy chcesz kontynuować mimo to?',
  'dialog.large_room_sysaudio.title': 'Udostępniaj dźwięk systemowy w dużym pokoju',
  'dialog.large_room_sysaudio.message':
    'Udostępnianie dźwięku systemowego obsługuje zarówno gości lokalnych, jak i zdalnych. Zalecamy maksymalnie 4 gości, aby uniknąć przeciążeń lub opóźnień.\n\nCzy chcesz kontynuować mimo to?',
  'dialog.continue': 'Kontynuuj',
  'connect.slot_guide.aria': 'Otwórz przewodnik po zalecanych limitach gości',
  'connect.slot_guide.title': 'Zalecany limit gości',
  'connect.slot_guide.body':
    'Im więcej gości połączy się z pokojem, tym bardziej staje się on niestabilny.\n\n• Wspólne YouTube: do 32 gości\n• Lokalne pliki: do 8 gości\n• Dźwięk systemowy: zalecane maksymalnie 4 gości',
  'toast.sync_reset': 'Offset synchronizacji wyzerowany',
  'toast.stop_sent': 'Wysłano żądanie zatrzymania odtwarzania',
  'toast.auto_play_canceled': 'Automatyczne odtwarzanie anulowane',
  'toast.playing_in_3s': 'Odtwarzanie rozpocznie się za 3 sekundy...',
  'toast.hprecision_sync': 'Synchronizacja o wysokiej precyzji: wczytywanie audio...',
  'toast.file_ready': 'Plik jest gotowy!\nNaciśnij przycisk Odtwórz.',
  'toast.host_sync_not_recommended': 'Zalecamy regulację synchronizacji bezpośrednio na urządzeniu gościa',
  'toast.host_sync_requested':
    'Zażądano precyzyjnej synchronizacji. Skonfiguruj ręczną synchronizację na urządzeniu gościa.',
  'toast.sync_not_in_system_audio': 'Synchronizacja nie jest obsługiwana w trybie udostępniania dźwięku systemowego',
  'toast.sync_not_ready': 'Niegotowe. Spróbuj ponownie za chwilę',
  'toast.host_ad': 'Host pokoju prawdopodobnie ogląda teraz reklamę',
  'toast.yt_sync_start': 'Synchronizowanie...',
  'toast.yt_sync_done': 'Synchronizacja zakończona',
  'toast.yt_rendezvous_start': 'Synchronizacja z hostem...',
  'toast.yt_rendezvous_done': 'Synchronizacja zakończona',
  'toast.yt_rendezvous_host_paused': 'Host wstrzymał odtwarzanie: wyrównano tylko aktualną pozycję',
  'toast.yt_rendezvous_no_data': 'Brak danych odtwarzania od hosta. Spróbuj ponownie za chwilę',
  'toast.yt_rendezvous_timeout': 'Synchronizacja nie powiodła się. Spróbuj ponownie za chwilę',
  'toast.yt_host_sync_sent': 'Wysłano sygnał automatycznej synchronizacji',
  'toast.yt_manual_sync_prompt':
    'Właśnie wykonano automatyczną synchronizację.\nJeśli wciąż słyszysz opóźnienie, dostosuj wartość ręcznie.',
  'toast.playlist_ended': 'Playlista dobiegła końca',
  'toast.operator_required': 'Tylko administrator może zmieniać ustawienia dźwięku',
  'toast.connection_closing': 'Połączenie przerwane. Ustawienia nie zostały zsynchronizowane',
  'toast.host_changed_setting': 'Ustawienia zostały zmienione',
  'toast.session_mismatch': 'Niezgodność sesji. Ponawianie transferu pliku...',
  'toast.receiving_pct': 'Odbieranie... {{pct}}%',

  // ─── Transfer ────────────────────────────────────────────────────
  'transfer.demo_loading': 'Wczytywanie utworu demo z serwera...',
  'transfer.demo_loaded': 'Utwór demo gotowy.',
  'transfer.demo_load_fail': 'Nie udało się wczytać utworu demo:',
  'transfer.demo_loading_short': 'Wczytywanie utworu demo...',
  'transfer.file_requesting': 'Żądanie pliku w toku...',
  'transfer.file_sending': 'Wysyłanie pliku w toku...',
  'transfer.file_wait': 'Oczekiwanie na plik...\nProszę czekać',
  'transfer.preload_done': 'Użycie wstępnie wczytanego pliku!',
  'transfer.preload_waiting': 'Oczekiwanie na zakończenie pre-loadu...',
  // Remote file sharing (R2-backed encrypted transfer)
  'share.remote.encrypting': 'Szyfrowanie pliku dla gości zdalnych...',
  'share.remote.uploading': 'Przesyłanie zaszyfrowanego pliku w toku...',
  'share.remote.upload_ready': 'Zdalne udostępnianie gotowe',
  'share.remote.waiting': 'Oczekiwanie na plik zdalny...',
  'share.remote.downloading': 'Pobieranie pliku zdalnego w toku...',
  'share.remote.upload_failed': 'Udostępnianie zdalne nie powiodło się: {{msg}}',
  'share.remote.download_failed': 'Pobieranie pliku zdalnego nie powiodło się: {{msg}}',
  'share.remote.timeout': 'Przekroczono limit czasu udostępniania zdalnego',
  'share.remote.too_large': 'Plik przekracza dozwolony limit (maks. 200MB)',
  'share.remote.network_error': 'Błąd sieci podczas udostępniania zdalnego',
  'share.remote.rate_limited': 'Zbyt wiele żądań udostępniania zdalnego.\nSpróbuj ponownie za chwilę',
  'share.remote.auth_failed': 'Uwierzytelnianie udostępniania zdalnego nie powiodło się. Sprawdź ustawienia Workera.',
  'share.remote.expired': 'Link do pliku zdalnego wygasł.\nUdostępnij go ponownie.',
  'share.remote.unavailable': 'Udostępnianie zdalne niedostępne.\nSpróbuj ponownie za chwilę.',
  'transfer.preload_fail': 'Pre-load nie powiódł się. Ponowne wczytywanie...',
  'transfer.download_finishing': 'Kończenie pobierania...',
  'transfer.preparation_delayed': 'Opóźnienie wczytywania...\nŻądanie przywrócenia wysłane do hosta.',
  'transfer.receiving_0pct': 'Odbieranie w toku... 0%',
  'transfer.check_conn_type': 'Sprawdzanie typu połączenia...',
  'transfer.preload_pending': 'Oczekiwanie na pre-load: {{name}}',
  'transfer.waiting_recovery': 'Oczekiwanie na przywrócenie: {{name}}',
  'transfer.preparing_name': 'Przygotowanie: {{name}}',
  'transfer.recovery_failed': 'Odzyskanie pliku nie powiodło się. Spróbuj ponownie.',

  // ─── Error ───────────────────────────────────────────────────────
  'error.https_required': 'Wymagany protokół HTTPS: ta funkcja wymaga bezpiecznego połączenia.',
  'error.browser_update': 'Zaktualizuj swoją przeglądarkę (iOS 15.2+, Chrome 86+ lub nowszą)',
  'error.load_failed': 'Wczytywanie nie powiodło się: {{msg}}',
  'error.audio_decode_fail': 'Dekodowanie audio nie powiodło się!',
  'error.audio_memory': 'Wczytywanie dźwięku do pamięci...',
  'error.decode_timeout': 'Dekodowanie „{{name}}” trwa zbyt długo.\nPrzejście do kolejnego utworu...',
  'error.local_decode_wait': 'To urządzenie nie mogło zdekodować utworu.\nOczekiwanie na kolejny utwór.',
  'error.all_tracks_failed': 'Brak odtwarzalnych utworów na playliście.',
  'error.audio_engine_prepare': 'Nie można uruchomić silnika audio',
  'error.session_create_fail': 'Nie udało się utworzyć pokoju',
  'error.network_generic': 'Wykryto błąd sieci',
  'error.host_unreachable': 'Host pokoju wydaje się być nieosiągalny',
  'error.host_disconnected': 'Połączenie z hostem pokoju przerwane',
  'error.host_conn_error': 'Błąd połączenia z hostem pokoju',
  'error.connect_failed': 'Połączenie nie powiodło się',
  'error.peer_not_ready': 'Połączenie peer nie jest yet gotowe',
  'error.network_init_failed': 'Inicjalizacja sieci nie powiodła się',
  'error.no_host_id': 'Identyfikator hosta pokoju nie został znaleziony',
  'error.peer_unavailable': 'Pokój nie został znaleziony.\nZweryfikuj wprowadzony kod.',
  'error.host_left': 'Host pokoju zamknął sesję',
  'error.network_issue': 'Wykryto problem z siecią.\nZweryfikuj swoje połączenie internetowe.',
  'error.signal_server_fail': 'Nie można połączyć się z serwerem sygnalizacyjnym.\nSpróbuj ponownie za chwilę.',
  'error.server_disconnected': 'Odłączono od serwera.',
  'error.session_id_unavailable': 'Identyfikator pokoju jest niedostępny.\nSpróbuj ponownie.',
  'error.webrtc_failed': 'Połączenie WebRTC nie powiodło się.\nSprawdź ustawienia przeglądarki.',
  'error.browser_unsupported':
    'Ta przeglądarka nie obsługuje połączeń w czasie rzeczywistym.\nUżyj najnowszej wersji Chrome lub Safari.',
  'error.ssl_required': 'Wymagane jest bezpieczne połączenie (HTTPS).',
  'error.room_password_required': 'Ten pokój wymaga hasła przy wejściu.',
  'error.room_password_invalid': 'Niepoprawne hasło.',
  'error.room_password_auth_timeout': 'Niestabilna sieć. Spróbuj ponownie wpisać hasło.',

  // ─── Dialog ──────────────────────────────────────────────────────
  'dialog.return_home_title': 'Wyjdź z pokoju',
  'dialog.return_home_msg': 'Czy chcesz powrócić do ekranu startowego?',
  'dialog.return_home_detail': 'Połączenie z obecnym pokojem zostanie przerwane.',
  'dialog.leaving_session': 'Wyjście w toku...',
  'dialog.background_resume_title': 'Aplikacja przeszła w tło',
  'dialog.background_resume_message':
    'Jeśli zauważysz problemy z synchronizacją lub brak dźwięku, zrestartuj całkowicie aplikację.',
  'dialog.background_resume_sync_fallback_message':
    'Jeśli zauważysz problemy z synchronizacją lub brak dźwięku, zrestartuj całkowicie aplikację.',
  'dialog.got_it': 'Rozumiem',
  'dialog.continue_using': 'Kontynuuj korzystanie',
  'dialog.leave_session': 'Wyjdź z pokoju',
  'dialog.sw_update_title': 'Nowa aktualizacja',
  'dialog.sw_update_msg': 'Dostępna jest nowa wersja. Odśwież stronę, aby ją zastosować.',
  'dialog.reconnect_ask': 'Czy chcesz spróbować połączyć się ponownie?',
  'dialog.reconnect': 'Połącz ponownie',
  'dialog.go_back': 'Wróć',
  'dialog.room_password_title': 'Wpisz hasło do pokoju',
  'dialog.room_password_msg': 'Aby dołączyć do tego pokoju, wymagane jest hasło.',
  'dialog.room_password_retry_msg': 'Wprowadzone hasło jest niepoprawne. Spróbuj ponownie.',
  'dialog.room_password_timeout_msg': 'Przekroczono limit czasu połączenia. Wpisz hasło ponownie.',
  'dialog.room_password_placeholder': '8-cyfrowe hasło',
  'dialog.session_lost_msg':
    'Aplikacja przeszła w tryb oszczędzania energii lub straciła połączenie, co przerwało łączność z pozostałymi urządzeniami. Zrestartuj, aby uruchomić nową sesję.',
  'dialog.session_lost_btn': 'Uruchom ponownie',

  // ─── HTML Blocks (used with data-i18n-html) ────────────────────
  'help.install_app_html':
    'Lubisz MUSIXQUARE? Można go zainstalować jako aplikację.<ul class="help-list"><li><strong>iOS:</strong> Udostępnij → Dodaj do ekranu głównego</li><li><strong>Android:</strong> Menu → Zainstaluj aplikację</li><li><strong>Windows:</strong> Zainstaluj ikonę w pasku adresu</li><li><strong>macOS:</strong> Udostępnij → Dodaj do Docka</li></ul>',
  'help.local_recommended_html':
    'Udostępnianie dźwięku jest najbardziej stabilne w tej samej sieci.<ul class="help-list"><li>Goście zdalni również mogą korzystać ze wszystkich funkcji.</li><li>Goście zdalni korzystają z węzłów Cloudflare, co może zwiększyć opóźnienie lub przerwać przesył w zależności od sieci.</li><li>Używanie VPN lub zabezpieczonych sieci firmowych może destabilizować połączenie.</li></ul>',
  'setup.how_to_connect_html':
    'Wprowadź <strong>6-cyfrowy kod</strong> od hosta, aby się połączyć.<ul class="help-list"><li><strong>Host (Gospodarz):</strong> “Utwórz pokój” → Sprawdź kod</li><li><strong>Gość (Uczestnik):</strong> “Dołącz do pokoju” → Wpisz kod</li><li>Limit połączonych urządzeń można zmienić w <strong>zakładce „Połączenie”</strong>.</li></ul>',
  'player.play_speakers_html':
    'Każdy gość wybiera <strong>rolę (kanał wyjściowy)</strong>.<ul class="help-list"><li><strong>Głośnik centralny:</strong> wyjście stereo (domyślne)</li><li><strong>Głośnik lewy:</strong> wyjście kanału L</li><li><strong>Głośnik prawy:</strong> wyjście kanału R</li><li><strong>Subwoofer:</strong> wyjście miksu niskich częstotliwości</li></ul>Role możesz zmienić w dowolnym momencie w <strong>Ustawieniach</strong>.',
  'player.play_media_action_html':
    'Host ma 3 opcje odtwarzania.<ul class="help-list"><li><strong>Załaduj plik lokalny:</strong> Wybierz utwory ze swojego urządzenia. Wspiera synchronizację o wysokiej precyzji</li><li><strong>YouTube razem:</strong> Wspiera wideo i playlisty. Brak rozdziału kanałów i efektów zaawansowanych</li><li><strong>Udostępnianie dźwięku systemowego:</strong> Dostępne tylko wtedy, gdy host korzysta z Chrome na Windows lub Mac</li></ul>',
  'help.need_help_html':
    '<ul class="help-list"><li><strong>Wpisałem kod, ale nie mogę się połączyć:</strong> Zweryfikuj kod i spróbuj ponownie za chwilę. Sieci korporacyjne, szkolne lub VPN mogą blokować połączenie.</li><li><strong>Niestabilne połączenie:</strong> Zaleca się, aby goście lokalni znajdowali się blisko routera Wi-Fi, a goście zdalni korzystali ze stabilnej sieci.</li><li><strong>Inne problemy:</strong> Zweryfikuj stan sieci i zrestartuj aplikację na wszystkich urządzeniach. W przypadku innych pytań, zapoznaj się z sekcją <a href="/faq?lang=pl" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Najczęściej zadawanych pytań (FAQ)</a> lub skontaktuj się z nami poprzez adres email na dole strony.</li></ul>',
  'chat.peer_connected': '{{name}} wszedł do pokoju',
  'chat.peer_disconnected': '{{name}} wyszedł z pokoju',
  'chat.empty_html': 'Brak wiadomości.<br>Napisz pierwszą wiadomość!',
  'chat.msg_truncated': 'Wiadomość została skrócona do {{max}} znaków',
  'chat.scroll_to_bottom': 'Przewiń na dół',
  'setup.brand_tagline_html': 'Słuchajcie razem, gdziekolwiek.<br>Doskonałe wrażenia dźwiękowe.',
  'setup.invite_share_desc_html':
    'Połącz bezprzewodowo wiele urządzeń,<br>aby stworzyć potężny system audio.<br>Użyj 6-cyfrowego kodu, aby się połączyć.',
  'setup.play_together_desc_html':
    'Kiedy host odtwarza utwór,<br>wszystkie połączone urządzenia grają synchronicznie.<br>Kliknij „Synchronizuj”, aby wyrównać audio.',
  'setup.spatial_audio_desc_html':
    'Ustaw rolę dla każdego urządzenia.<br>Odtwarzaj lewy i prawy dźwięk osobno<br>i poczuj głęboki bas dzięki subwooferowi.',
  'setup.advanced_audio_desc_html':
    'Synchronizuj z ustawieniami hosta<br>i zastosuj w całym systemie pogłos, EQ,<br>wirtualne efekty i wiele innych.',
  'youtube.enter_link_desc_html': 'Można odtwarzać tylko wideo publiczne.',

  // ─── System Audio Sharing ─────────────────────────────────────
  'system_audio.button': 'Dźwięk systemowy',
  'system_audio.sharing': 'Udostępnianie dźwięku systemowego',
  'system_audio.receiving': 'Odbieranie dźwięku systemowego',
  'system_audio.capture_denied': 'Udostępnianie dźwięku zostało anulowane.',
  'system_audio.no_audio_track': 'Opcja „Udostępnij dźwięk” nie została włączona.\nSpróbuj ponownie.',
  'system_audio.stop': 'Zatrzymaj udostępnianie',
  'system_audio.started':
    'Dźwięk systemowy może charakteryzować się niewielkim opóźnieniem.\nZalecamy hostowi maksymalne zmniejszenie głośności swojego urządzenia.',
  'system_audio.stopped': 'Udostępnianie dźwięku systemowego zostało zakończone.\nWznawianie playlisty.',
  'system_audio.many_devices_warning':
    'Dźwięk systemowy działa optymalnie przy maksymalnie 4 gościach.\nWiększa liczba może powodować opóźnienia lub zacinanie.',
  'system_audio.host_channel_locked': 'Nie można zmienić ról głośników podczas udostępniania dźwięku systemowego.',
  'system_audio.effects_guest_only':
    'Udostępnianie dźwięku systemowego aktywne.\nZaawansowane efekty dźwiękowe mają zastosowanie wyłącznie do urządzeń gości.',
  'system_audio.desktop_only': 'Dostępne wyłącznie przy użyciu Chrome na komputerach stacjonarnych Windows lub Mac.',
  'system_audio.receive_failed':
    'Nie udało się odebrać dźwięku systemowego.\nPoproś hosta o ponowne uruchomienie udostępniania.',
  'system_audio.remote_receive_limit':
    'Odbiór zdalnego dźwięku systemowego został automatycznie przerwany po 2 godzinach ze względów bezpieczeństwa.\nWejdź ponownie do pokoju, aby kontynuować słuchanie.',

  // ─── Legal ────────────────────────────────────────────────────────
  'legal.title': 'Informacje prawne i Warunki świadczenia usług',
  'legal.content_html':
    '<strong>Polityka prywatności</strong><br>W tej samej sieci lokalnej dane sesji przesyłane są bezpośrednio między urządzeniami; tylko szczegóły adresu IP i parametry połączenia przechodzą przez serwer sygnalizacyjny. Zdalne udostępnianie plików oraz dźwięku systemowego przesyła dane bezpiecznie przez sieć Cloudflare; pliki zdalne są szyfrowane, przechowywane przez maksymalnie 24 godziny i zapisywane bez klucza deszyfrującego na naszych serwerach. MUSIXQUARE nie przegląda, nie analizuje ani nie przechowuje Twoich danych poza zakresem ściśle niezbędnym do świadczenia swoich funkcji. <a href="/privacy?lang=pl" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Dowiedz się więcej</a><br><br><strong>Ostrzeżenie o prawach autorskich</strong><br>Treści udostępniane za pośrednictwem MUSIXQUARE należą do ich odpowiednich właścicieli praw. Funkcje udostępniania dźwięku systemowego oraz plików są przeznaczone wyłącznie do użytku osobistego i prywatnego. Użytkownicy ponoszą pełną odpowiedzialność za nieuprawnione rozpowszechnianie lub publiczne przesyłanie treści chronionych prawem autorskim. Udostępnianie z YouTube wykorzystuje oficjalną strukturę odtwarzacza API YouTube; wyświetlenia oraz zarządzanie prawami podlegają ściśle polityce platformy i preferencjom ustawionym przez twórców. <a href="/terms?lang=pl" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Dowiedz się więcej</a><br><br><strong>Wyłączenie odpowiedzialności</strong><br>MUSIXQUARE jest świadczony bezpłatnie. Niektóre funkcje opierają się na zewnętrznych usługach stron trzecich (takich jak YouTube lub Cloudflare), w związku z czym mogą wystąpić tymczasowe ograniczenia funkcjonalne w przypadku zmian lub przerw w tych usługach. Synchronizacja dźwięku w czasie rzeczywistym oraz jakość strumieniowania zdalnego zależą w dużej mierze od stabilności sieci, polityki używanej przeglądarki oraz wydajności poszczególnych urządzeń. W przypadku braku połączenia z siecią Wi-Fi mogą obowiązywać opłaty za transmisję danych w sieci komórkowej zgodnie z taryfą operatora. <a href="/terms?lang=pl" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Dowiedz się więcej</a><br><br><strong>© 2026 MUSIXQUARE</strong><br><a href="/about" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">O nas</a> · <a href="/history" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Historia</a> · <a href="/designsystem" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Design System</a> · <a href="https://github.com/hiefny/MUSIXQUARE" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">GitHub</a> · <a href="https://discord.gg/PmmFhGTBsX" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">Discord</a><br><a href="mailto:contact@musixquare.com" data-copy-email="contact@musixquare.com" style="color:inherit;text-decoration:none">contact@musixquare.com</a>',
};

export default pl;
