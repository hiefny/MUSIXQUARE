# MUSIXQUARE Trademark Policy

- **Effective date:** 2026-08-22
- **Trademark owner and policy administrator:** CHOI HYEONSEUNG
- **Contact:** [contact@musixquare.com](mailto:contact@musixquare.com)

MUSIXQUARE is open-source software. Its source code is licensed under the GNU
Affero General Public License version 3 or later (`AGPL-3.0-or-later`). That
copyright license permits use, study, modification, distribution, network
operation, and commercial use of covered software, subject to its terms.

The AGPL does not grant permission to use the MUSIXQUARE Marks as the name,
branding, or source identifier of another product or service.

In this policy, the **Rights Holder** means CHOI HYEONSEUNG.

## Official Project Channels

- Website: [musixquare.com](https://musixquare.com)
- Source repository: [github.com/hiefny/MUSIXQUARE](https://github.com/hiefny/MUSIXQUARE)
- Contact: [contact@musixquare.com](mailto:contact@musixquare.com)

## MUSIXQUARE Marks

The “MUSIXQUARE Marks” include, to the extent controlled by the Rights Holder:

- the names `MUSIXQUARE` and `뮤직스퀘어`;
- the custom MUSIXQUARE wordmark and logo lockups;
- the official MUSIXQUARE app icon; and
- official source-identifying combinations of those names, logos, or icons;
  and
- the distinctive, non-functional overall visual presentation of the official
  MUSIXQUARE product, including its particular selection, combination, and
  arrangement of custom interface artwork, visual hierarchy, component
  geometry, typography, color treatment, motion, transitions, and recurring
  screen compositions, where and to the extent that presentation functions as
  a source identifier and is protected by applicable law.

This definition does not claim exclusive rights in generic music symbols,
individual colors, fonts, design tokens, spacing values, standard icons,
common interface components or patterns, functional designs, workflows,
navigation conventions, or third-party assets by themselves. The absence of
exclusive rights in an individual element does not prevent a distinctive
combination of elements, considered as a whole, from being protected to the
extent recognized by applicable law.

## Uses That Do Not Require Separate Permission

Subject to applicable law, you may:

- refer truthfully to MUSIXQUARE in plain text;
- state that software is “based on MUSIXQUARE”, “a fork of MUSIXQUARE”, or
  “compatible with MUSIXQUARE”;
- preserve copyright, license, attribution, source-history, filename, and
  platform-generated fork notices;
- link to the official project or source repository; and
- show the Marks as reasonably necessary in factual reporting, reviews,
  education, documentation, comparisons, or screenshots discussing the
  official project.

Such use must not make a MUSIXQUARE Mark the primary identity of your product
or imply affiliation, sponsorship, endorsement, operation, or official status.
Your own identity should be more prominent than any MUSIXQUARE reference.

Nothing in this policy limits nominative, descriptive, fair, or other uses
allowed without permission under applicable law.

## Public Forks and Hosted Services

You do not need trademark permission merely to use, modify, distribute, host,
support, or charge for AGPL-licensed software under a distinct identity.

Replacing only the MUSIXQUARE name, logo, icon, or colors is not necessarily
sufficient to create a distinct identity, and a logo-only reskin is not a safe
harbor. If the remaining public-facing presentation reproduces or imitates a
protected MUSIXQUARE source-identifying overall visual presentation in a
manner likely to cause confusion, the operator must take sufficient steps to
eliminate the likely confusion—which may require redesign—or obtain written
permission, subject to applicable law.

An independent public fork or hosted service should:

- use its own product name, logo, app icon, favicon, domain, and account names;
- use its own distinguishable visual system and overall user-facing
  presentation rather than a logo-only reskin of the official interface;
- identify itself clearly as independent and unofficial;
- replace MUSIXQUARE operator, support, legal-policy, canonical, social-preview,
  search, manifest, invite, and service metadata with its own accurate details;
- preserve license, copyright, source-history, and other legally required
  notices; and
- provide the Corresponding Source required by the AGPL for the version that
  users actually receive or interact with over a network.

Truthful attribution may remain. Recommended wording:

> [PROJECT] is an independent project based on MUSIXQUARE. It is not affiliated
> with, sponsored by, endorsed by, or operated by the MUSIXQUARE project or its
> Rights Holder.

See [BRAND_POLICY.md](./BRAND_POLICY.md) for a practical implementation
checklist.

## Uses Requiring Written Permission

Written permission is required to use a MUSIXQUARE Mark:

- as the name or primary branding of another application, distribution,
  hosted service, package, domain, account, organization, or event;
- in a third-party app icon, favicon, PWA identity, logo, loading screen,
  advertisement, store listing, or social/search preview;
- in a confusingly similar name, source-identifying logo, domain, or branding,
  to the extent protected by applicable law;
- to reproduce or imitate a protected, distinctive, non-functional overall
  MUSIXQUARE visual presentation in a manner likely to cause confusion as to
  source, affiliation, sponsorship, endorsement, or operation;
- to describe a modified or third-party build as an official MUSIXQUARE
  release; or
- in a manner suggesting that the Rights Holder or MUSIXQUARE project operates,
  sponsors, approves, or supports a third-party product or service.

Requests may be sent to
[contact@musixquare.com](mailto:contact@musixquare.com). Permission must be
express and in writing; silence or non-enforcement is not permission.

## Open-Source Artwork and Trademark Use

A logo, icon, wordmark, SVG, stylesheet, or other asset included in an
AGPL-covered source release may be copied, modified, and distributed under the
applicable copyright permissions of the AGPL. Those copyright permissions do
not grant the separate right to use a MUSIXQUARE Mark as a trademark or to
identify a third-party product or service.

Likewise, permission to copy or modify interface code, styles, layouts, or
assets under the AGPL is not permission to deploy them as a confusingly
similar, source-identifying public presentation. This distinction concerns
trademark and related source-confusion rights, not the copyright permissions
granted by the AGPL.

Forks may preserve these materials in source history and required legal
notices, but must replace their public-facing use as product branding unless
separately authorized or otherwise permitted by law.

## Scope, Existing Rights, and Updates

This policy does not amend the AGPL, withdraw copyright permissions already
granted for any version, or retroactively alter an express written trademark
permission. Publication of this policy does not create rights that do not
otherwise exist, and does not waive any trademark, unfair-competition,
passing-off, or other rights or remedies that may exist.

A written exception applies only to the recipient and scope stated in that
exception. Subject to applicable law, a delay or decision not to enforce in one
situation does not itself grant trademark permission, waive rights, or
authorize later or expanded use. Only express written permission binds the
Rights Holder, according to its stated terms.

This policy may be updated prospectively. Existing express written permissions
remain governed by their own terms.
