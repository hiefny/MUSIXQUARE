...
function assertFile(root, path, label, suffix) {
...
function assertFileDigest(root, path, digest, label) {
...
function discoverSqlContracts(root) {
...
function assertSameInventory(label, discovered, declared) {
...
export function assertD1MigrationContract({
  ...
  for (const database of manifest.databases) {
    ...
    const databaseMigrationIds = new Set();
    ...
    } else if (!databaseMigrationIds.has(database.baselineMigration)) {
      throw new Error(
...
export function assertD1MigrationManifestAppendOnly({ previousManifest, currentManifest }) {
...
export function trackedD1PathsForDatabase(manifest, databaseName) {
