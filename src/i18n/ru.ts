...
const ru: Record<I18nKey, string> = {
  ...
  'setup.room_type_hint':
    'Коды, начинающиеся с 0, ведут в постоянные PRO-комнаты, а с 1–9 — во временные обычные комнаты.',
  'setup.standard_room_summary':
    'Обычная комната · Временная и закрывается, когда организатор выходит. Организатор управляет воспроизведением и правами; может потребоваться необязательный 8-значный пароль комнаты.',
  'setup.pro_room_summary':
    'PRO-комната · Плейлист и права сохраняются после выхода всех участников. Ею управляют владельцы и администраторы; требуется 8-значный пароль PRO.',
  'setup.center_role_guide':
    'Это устройство подключилось как центральный динамик. Роль можно изменить в Настройки → Аудио → Роль этого устройства.',
  'setup.center_role_open_settings': 'Открыть настройки роли',
  ...
  'help.local_memory_notice':
    'Файл обрабатывается в оперативной памяти устройства. Для удалённой и PRO-передачи действует лимит 200 МиБ; при нехватке памяти могут не открыться и меньшие файлы.',
  ...
  'system_audio.preflight_limits':
    'Chromium на компьютере (Chrome, Edge и др.) · до {{count}} устройств · до {{hours}} часов',
