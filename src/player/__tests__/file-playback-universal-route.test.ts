...
function makeMp3Frame(): Uint8Array<ArrayBuffer> {
...
function mp3File(name = 'fixture.mp3', mime = 'audio/mpeg'): File {
  return new File([blobPart(mp3Bytes())], name, { type: mime });
}

function m4aFile(name = 'fixture.m4a', mime = 'audio/mp4'): File {
  return new File([blobPart(buildM4aAacFixture().bytes)], name, { type: mime });
}
...
function aacBytes(): Uint8Array<ArrayBuffer> {
...
function aacFile(name = 'fixture.aac', mime = 'audio/aac'): File {
  return new File([blobPart(aacBytes())], name, { type: mime });
}
...
function id3PrefixedAacFile(): File {
  return new File(
    [
      blobPart(id3v2Tag(2, 3)),
      blobPart(id3v2Tag(3, 5)),
      blobPart(id3v2Tag(4, 4, true)),
      blobPart(aacBytes()),
    ],
    'misnamed.mp3',
    { type: 'audio/mpeg' },
...
function id3PrefixedMp3File(name = 'fixture.aac', mime = 'audio/aac'): File {
  ...
  return new File([blobPart(emptyId3v24), blobPart(mp3Bytes())], name, { type: mime });
}
...
describe('default-off universal bounded audio routes', () => {
  ...
  it.each([
    ...
    await expect(
      createBlobFilePlaybackSource(
        blobOptions(new File([blobPart(bytes())], 'partial.aac', { type: 'audio/aac' }), {
          boundedRoutePolicy: FILE_PLAYBACK_UNIVERSAL_V1_BOUNDED_ROUTE_POLICY,
          decodeOrdinaryAudio,
          backendFactories: {
            createStreamingAacSource:
