...
function Settings({
  ...
  const reverbOptions: readonly UiKitReverb[] = ['Off', 'Studio', 'Arena', 'Advanced'];
  return (
    <>
      ...
      <div className="mq-card">
        ...
        <div className="mq-seg" style={{ marginBottom: 14 }}>
          {reverbOptions.map((name) => (
            <button
              key={name}
              className={reverb === name ? 'active' : ''}
              onClick={() => onReverb(name)}
            >
              {name}
            </button>
          ))}
      ...
      <div className="mq-card">
        ...
        <div className="mq-seg">
          {['Off', 'Bright', 'Warm', 'Advanced'].map((name) => (
            <button key={name} className={name === 'Warm' ? 'active' : ''}>
              {name}
            </button>
          ))}
        </div>
