...
const isDomainEvent = (name: string): boolean => name.includes(':') && !name.startsWith('state:');
...
function collect(
  ...
  while ((match = expression.exec(text))) {
    ...
    const location = `${relative(ROOT, file)}:${lineOf(text, match.index)}`;
    const locations = target.get(name);
    if (locations) locations.push(location);
    else target.set(name, [location]);
  }
}

const emits = new Map<string, string[]>();
const listens = new Map<string, string[]>();
...
const deadEmits = [...emitNames].filter((n) => !listenNames.has(n) && !ALLOWLIST.has(n)).sort();
