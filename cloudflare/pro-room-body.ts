...
type BodyStopOutcome = { kind: 'timeout' } | { kind: 'aborted' };
...
type BodyReadResult =
  ...
  | { error: 'invalid' | 'too-large' | 'timeout' | 'aborted' };

interface CancellableBody {
  cancel?: (reason?: unknown) => unknown;
  body?: CancellableBody | null;
}

export async function readBodyBytesLimited(
  request: Request,
  maxBytes: number,
  timeoutMs: number,
  ...
  let totalBytes = 0;
  let stop!: (outcome: BodyStopOutcome) => void;
  const stopped = new Promise<BodyStopOutcome>((resolve) => {
    stop = resolve;
  });
...
export function cancelReadableBody(
  bodyOrReader: CancellableBody | null | undefined,
  reason: unknown,
): void {
