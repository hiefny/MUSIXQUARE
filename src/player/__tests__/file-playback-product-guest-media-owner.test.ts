...
const R2_RECORD_BYTES = 8 * 1024 * 1024;
...
>;
type GuestRuntimeFunction<Key extends keyof FilePlaybackProductGuestMediaOwnerRuntimeForTests> =
  NonNullable<FilePlaybackProductGuestMediaOwnerRuntimeForTests[Key]>;
type GuestParticipantFixture = ReturnType<GuestRuntimeFunction<'createParticipant'>>;
...
function r2RecordOffer(context: Readonly<FilePlaybackProductSessionRouterConnectionContext>) {
  return createR2RecordFileMediaSourceOfferV2({
    sessionId: context.sessionId,
    connectionId: context.connectionId,
    prepareId: PREPARE_ID,
    prepareRevision: 1,
    ...
    setId: '98000000-0000-4000-8000-000000000003',
    encodedSize: R2_RECORD_BYTES + 7,
    recordSize: R2_RECORD_BYTES,
    recordCount: 2,
    recordObjectIds: '98000000-0000-4000-8000-000000000005,98000000-0000-4000-8000-000000000006',
    keyB64: btoa(String.fromCharCode(...new Uint8Array(32).fill(11))),
    noncePrefixB64: btoa(String.fromCharCode(...new Uint8Array(8).fill(13))),
    name: 'remote-records.flac',
    mime: 'audio/flac',
    expiresAtRoomTimeMs: 10_000,
  });
}

function r2RecordSourceFixture(
  offer: ReturnType<typeof r2RecordOffer>,
  readAt: (offset: number, length: number, signal: AbortSignal) => Promise<Uint8Array> = async (
    _offset,
    length,
  ...
  const source = {
    kind: 'r2-records' as const,
    size: offer.encodedSize,
    identity: offer.sourceIdentity,
    metadata: Object.freeze({ name: offer.name, mime: offer.mime }),
    readAt: read,
    close,
  } as unknown as R2RecordEncodedAudioSource;
...
function runtimeHarness(
  ...
  const acquireManifestAsset = vi.fn<GuestRuntimeFunction<'acquireManifestAsset'>>(
    async ({ operation }) => {
  ...
  const prepareManifestDecoderConstruction = vi.fn<
    GuestRuntimeFunction<'prepareManifestDecoderConstruction'>
  >(async ({ assetLease }) => {
  ...
  const retireManifestDecoderConstruction = vi.fn<
    GuestRuntimeFunction<'retireManifestDecoderConstruction'>
  >(async () => {
    manifestEvents.push('retire-construction');
    return true;
  });
  const stageManifestAssetSource = vi.fn<GuestRuntimeFunction<'stageManifestAssetSource'>>(
    async (options) => {
  ...
  const stageAssetSource = vi.fn<GuestRuntimeFunction<'stageAssetSource'>>(
    async (options: StageFilePlaybackAssetSourceOptions) => {
      const staged = stagedAssetSource(registry, options);
      candidateBackend = staged.backend;
      return staged;
    },
  ...
  >();
  const prepareWarmSource = vi.fn<GuestRuntimeFunction<'prepareWarmSource'>>(async (options) => {
    const asset = registry.snapshotForLease(ROOM_TOKEN, options.assetLease);
  ...
  const handoffWarmSource = vi.fn<GuestRuntimeFunction<'handoffWarmSource'>>(async (options) => {
  ...
  const retireWarmSource = vi.fn<GuestRuntimeFunction<'retireWarmSource'>>(async (authority) =>
    warmRecords.delete(authority),
  );
  const arm = vi.fn<GuestParticipantFixture['arm']>(async (intent: RendezvousArmIntent) =>
    freezeCanonical({
  ...
  const finalize = vi.fn<GuestParticipantFixture['finalize']>(
    async (intent: RendezvousFinalizeIntent) =>
      freezeCanonical({
        protocolVersion: 2 as const,
  ...
  const started = vi.fn<GuestParticipantFixture['started']>(async () =>
    freezeCanonical({
  ...
  const cancel = vi.fn<GuestParticipantFixture['cancel']>(async () => undefined);
  const recoveryRequired = vi.fn(() => physicalRecoveryRequired);
  const primeCandidate = vi.fn<GuestRuntimeFunction<'primeCandidate'>>(
    async (_manager, _port, _positionSeconds, signal) => {
  ...
  const pauseCurrent = vi.fn<GuestRuntimeFunction<'pauseCurrent'>>(
  ...
  const seekCurrent = vi.fn<GuestRuntimeFunction<'seekCurrent'>>(
    async (_manager, _port, intent) => ({
      status: 'scheduled' as const,
      reason: null,
      from: intent.from,
...
describe('FilePlaybackProductGuestMediaOwner', () => {
  ...
  it('validates, primes record zero, and transfers an R2 record source to registry lifetime', async () => {
    const h = setup();
    const offer = r2RecordOffer(h.context);
    const fixture = r2RecordSourceFixture(offer);
    const createSource = vi
      .spyOn(R2RecordEncodedAudioSource, 'create')
      .mockResolvedValue(fixture.source);
    ...
    try {
      ...
      const [options, signal] = createSource.mock.calls[0]!;
      expect(signal).toBeInstanceOf(AbortSignal);
      expect(options).toMatchObject({
        storageRoomId: '123456',
        setId: offer.setId,
        identity: SOURCE_ID,
        metadata: { name: 'remote-records.flac', mime: 'audio/flac' },
        secretDescriptor: {
          formatVersion: 2,
          objectId: offer.setId,
          plaintextSize: R2_RECORD_BYTES + 7,
          recordSize: R2_RECORD_BYTES,
          recordCount: 2,
        ...
        records: [
          {
            index: 0,
            objectId: '98000000-0000-4000-8000-000000000005',
            plaintextSize: R2_RECORD_BYTES,
            encryptedSize: R2_RECORD_BYTES + 16,
          },
          {
            index: 1,
            objectId: '98000000-0000-4000-8000-000000000006',
            plaintextSize: 7,
            encryptedSize: 23,
          },
        ],
      });
      expect(options.expiresAtEpochMs).toBeGreaterThan(Date.now());
      expect(fixture.read).toHaveBeenCalledOnce();
      expect(fixture.read).toHaveBeenCalledWith(0, 1, signal);
      expect(fixture.read.mock.invocationCallOrder[0]).toBeLessThan(
        h.runtime.stageAssetSource.mock.invocationCallOrder[0]!,
      );
      const stageOptions = h.runtime.stageAssetSource.mock.calls[0]![0];
      expect(h.registry.snapshotForLease(ROOM_TOKEN, stageOptions.assetLease)).toMatchObject({
        kind: 'r2-records',
        size: R2_RECORD_BYTES + 7,
        sourceIdentity: SOURCE_ID,
        name: 'remote-records.flac',
        mime: 'audio/flac',
      });
      ...
      await h.registry.retire(ROOM_TOKEN, stageOptions.assetLease);
  ...
  it('closes an in-flight R2 record source when its pre-admission operation is revoked', async () => {
    const h = setup();
    const offer = r2RecordOffer(h.context);
    let readSignal: AbortSignal | null = null;
    const fixture = r2RecordSourceFixture(offer, async (_offset, _length, signal) => {
      readSignal = signal;
      return new Promise<Uint8Array>((_resolve, reject) => {
        const rejectForAbort = () =>
          reject(signal.reason ?? new DOMException('superseded', 'AbortError'));
    ...
    try {
      ...
      expect((readSignal as unknown as AbortSignal).aborted).toBe(true);
      expect(h.registry.activeAssetCount(ROOM_TOKEN)).toBeNull();
      expect(h.runtime.stageAssetSource).not.toHaveBeenCalled();
      expect(
        h.runtime.sent.some((frame) => (frame as { kind?: string }).kind === 'source-ready'),
      ).toBe(false);
  ...
  it('keeps an admitted R2 record source open until terminal room close', async () => {
    const h = setup();
    const offer = r2RecordOffer(h.context);
    const fixture = r2RecordSourceFixture(offer);
    const createSource = vi
  ...
  it('aborts an AudioContext wait immediately on revoke without acquiring media', async () => {
    ...
    h.owner.revoke(h.context);
    await vi.waitFor(() => expect(h.runtime.peerClose).toHaveBeenCalledOnce());
    expectAbortedSignal(observedSignal);
    expect(h.runtime.r2Acquire).not.toHaveBeenCalled();
    expect(h.runtime.stageAssetSource).not.toHaveBeenCalled();
    expect(h.fatal).not.toHaveBeenCalled();
  });
  ...
  it('keeps exact guest preparation and rendezvous loading pending until matching render evidence', async () => {
    const loadingStateChanged = vi.fn();
    const h = setup({ onLoadingStateChange: loadingStateChanged });
    h.owner.onTimelineAdopted(stoppedTimelineEvent(h.context));
    const offer = peerOffer(h.context);
    ...
    const preparation = loadingStateChanged.mock.calls[0]?.[0];
    if (!preparation) throw new Error('Expected exact guest preparation loading projection');
    ...
    expect(loadingStateChanged).toHaveBeenCalledTimes(1);

    await runHostAttempt(h, h.state, SOURCE_ID, TRANSFER_ID, RENDEZVOUS_ID, 'bootstrap-stopped');

    const rendezvous = loadingStateChanged.mock.calls[1]?.[0];
    if (!rendezvous) throw new Error('Expected exact guest rendezvous loading projection');
    expect(rendezvous).toMatchObject({
      phase: 'pending',
      owner: 'guest-rendezvous',
    });
    expect(loadingStateChanged.mock.calls[2]?.[0]).toEqual({
      phase: 'settled',
    ...
    expect(loadingStateChanged.mock.calls[3]?.[0]).toEqual({
      phase: 'settled',
  ...
  it('keeps a late-join baseline alive when PAUSE cancels recovery before bootstrap commit', async () => {
    const h = setup();
    const baseline = await stageCancelablePreparedKind(h, 'baseline');
    const stageCountBeforeCancel = h.runtime.stageAssetSource.mock.calls.length;
    ...
    receiveHostWire(
      h,
      h.host.createWire(baseline.attemptLease, {
        kind: 'file-playback-cancel',
        rendezvousId: baseline.rendezvousId,
        reasonCode: 'remote-recovery-pause-transition',
      }),
    );
    h.host.retireAttempt(baseline.attemptLease);

    const paused = freezeCanonical({
      ...baseline.state,
      revision: baseline.state.revision + 1,
    });
    const stateLease = h.host.stageMedia({
      run: paused,
      sourceIdentity: SOURCE_ID,
    ...
    receiveHostWire(
      ...
      h.host.createWire(stateLease, {
        kind: 'file-playback-pause',
        expectedQueueItemId: baseline.state.queueItemId,
        expectedRunId: baseline.state.runId,
        expectedRevision: baseline.state.revision,
        atRoomTimeMs: 300,
      }),
    );

    h.host.commitMedia(stateLease);
    const timeline = activeTimeline(paused, 'paused', 0, 300);
  ...
  it('keeps a re-entrant SOURCE_READY cancellation retired before the following PAUSE', async () => {
    const h = setup();
    const staged = stageHostAttempt(
      h,
      h.state,
    ...
    const cancel = h.host.createWire(staged.attemptLease, {
      kind: 'file-playback-cancel',
      rendezvousId: RENDEZVOUS_ID,
      reasonCode: 'remote-recovery-pause-transition',
    });
    const armAcknowledge = vi.fn();
    const cancelAcknowledge = vi.fn();
    let reentered = false;
    h.runtime.cancel.mockImplementationOnce(async () => {
      h.runtime.setHasCurrentPort(false);
    });
    h.runtime.sendRequired.mockImplementation(async (_context, frame) => {
      ...
      if (!reentered && (frame as { kind?: string }).kind === 'source-ready') {
        reentered = true;
        const receivedArm = h.guest.receive(arm, h.guestConnection);
        if (!receivedArm.accepted || receivedArm.frame !== 'wire') {
          throw new Error('Expected re-entrant baseline ARM wire');
        }
        h.owner.adoptWireMessage(wireEvent(h.context, receivedArm), armAcknowledge);
        const receivedCancel = h.guest.receive(cancel, h.guestConnection);
        if (!receivedCancel.accepted || receivedCancel.frame !== 'wire') {
          throw new Error('Expected re-entrant baseline CANCEL wire');
        }
        h.owner.adoptWireMessage(wireEvent(h.context, receivedCancel), cancelAcknowledge);
        h.host.retireAttempt(staged.attemptLease);
      }
      return true;
    });
    ...
    startPreparation(h);
    await vi.waitFor(() => expect(cancelAcknowledge).toHaveBeenCalledOnce());
    ...
    expect(h.runtime.arm).not.toHaveBeenCalled();
    expect(
      h.runtime.stageAssetSource.mock.calls.length + h.runtime.handoffWarmSource.mock.calls.length,
    ).toBe(1);
    expect(
      h.runtime.sent.filter((frame) => (frame as { kind?: string }).kind === 'source-ready'),
    ).toHaveLength(1);
  ...
  it('retires a late-join baseline cleanly when STOP cancels recovery before bootstrap commit', async () => {
    const h = setup();
    const baseline = await stageCancelablePreparedKind(h, 'baseline');
    const stageCountBeforeCancel = h.runtime.stageAssetSource.mock.calls.length;
    h.runtime.cancel.mockImplementationOnce(async () => {
    ...
    receiveHostWire(
      ...
      h.host.createWire(baseline.attemptLease, {
        kind: 'file-playback-cancel',
        rendezvousId: baseline.rendezvousId,
        reasonCode: 'remote-recovery-stop-transition',
      }),
    );
    h.host.retireAttempt(baseline.attemptLease);
    ...
    h.host.commitStop(stateLease, baseline.state);
    const timeline = stoppedTimeline(stopped.revision, 300);
    h.owner.onTimelineUpdated(timelineUpdated(h.context, timeline));
    await vi.waitFor(() => expect(h.rendered).toHaveBeenCalledWith(timeline));

    expect(h.runtime.stageAssetSource).toHaveBeenCalledTimes(stageCountBeforeCancel);
    expect(h.runtime.stopCurrent).not.toHaveBeenCalled();
    expect(h.owner.currentRenderedDurationSeconds()).toBeNull();
    expect(h.fatal).not.toHaveBeenCalled();
    h.owner.revoke(h.context);
  });

  it('still fail-closes a terminal recovery reason that targets a run successor', async () => {
    const h = setup();
    const successor = await stageCancelablePreparedKind(h, 'successor');
    const cancel = h.host.createWire(successor.attemptLease, {
      kind: 'file-playback-cancel',
      rendezvousId: successor.rendezvousId,
      reasonCode: 'remote-recovery-pause-transition',
    });
    const received = h.guest.receive(cancel, h.guestConnection);
    if (!received.accepted || received.frame !== 'wire') {
      throw new Error('Expected terminal successor CANCEL wire');
    }
    const acknowledge = vi.fn();

    expect(() => h.owner.adoptWireMessage(wireEvent(h.context, received), acknowledge)).toThrow(
      FilePlaybackProductGuestMediaOwnerFatalError,
    );
