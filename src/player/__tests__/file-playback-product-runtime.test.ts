...
function requireCapture<T>(subject: Capture<T>, message: string): T {
  if (subject.value === null) throw new Error(message);
...
describe('FilePlaybackProductRuntime', () => {
  ...
  it('coalesces one exact next-file warm and aborts it before warming a replacement', async () => {
    ...
    const firstSignal = capture<AbortSignal>();
    ...
    const supersededSignal = requireCapture(firstSignal, 'first warm signal unavailable');
    expect(supersededSignal.aborted).toBe(true);
    firstGate.reject(supersededSignal.reason);
  ...
  it('detaches an abort-ignoring warm lane when its room ends', async () => {
    ...
    const staleSignal = capture<AbortSignal>();
    staleRoom.warmLocalTrack.mockImplementationOnce(
      (input: WarmFilePlaybackProductHostLocalTrackOptions) => {
        staleSignal.value = input.signal;
    ...
    setup.runtime.endRoom();
    const retiredSignal = requireCapture(staleSignal, 'stale warm signal unavailable');
    ...
    staleGate.reject(retiredSignal.reason);
  ...
  it('keeps at most two exact LAN guests direct and moves the shared third stream to R2', () => {
    const routers: ProductRouterHarness[] = [];
    const captured = capture<Readonly<FilePlaybackProductHostMediaOwnerOptions>>();
    ...
    setup.runtime.initializeBeforeProtocol();
    setup.runtime.beginHostRoom('bounded-delivery-host');
    const exactConnection = connection();
    routers[0]!.options.createHostMediaOwner(
      routerContext('host', {
        connection: exactConnection,
        suffix: 'bounded-delivery',
      }),
    );
    const ownerOptions = requireCapture(captured, 'bounded delivery owner options unavailable');
    const select = ownerOptions.selectBoundedTransport;
    const resolveStorageRoom = ownerOptions.resolveR2StorageRoomId;
    if (!select || !resolveStorageRoom) throw new Error('bounded delivery policy was not wired');

    const previousPeers = getState('network.connectedPeers');
    const previousCode = getState('network.sessionCode');
    const peer = (
      id: string,
      conn: DataConnection,
    ...
    try {
      ...
      setState('network.connectedPeers', [peer('one', exactConnection)]);
      expect(select(exactConnection)).toBe('peer-range');
      expect(resolveStorageRoom()).toBe('123456');

      const second = connection();
      setState('network.connectedPeers', [peer('one', exactConnection), peer('two', second)]);
      expect(select(exactConnection)).toBe('peer-range');

      const third = connection();
      setState('network.connectedPeers', [
        peer('one', exactConnection),
        peer('two', second),
        peer('three', third),
      ]);
      expect(select(exactConnection)).toBe('r2-records');
      expect(select(third)).toBe('r2-records');

      setState('network.connectedPeers', [peer('remote', exactConnection, 'remote')]);
      expect(select(exactConnection)).toBe('r2-records');
      setState('network.connectedPeers', []);
      expect(select(exactConnection)).toBe('r2-records');
    } finally {
      setState('network.connectedPeers', [...previousPeers]);
      setState('network.sessionCode', previousCode);
    }
  });

  it('retains the exact warm result and publishes it only after one late owner is READY', async () => {
    const routers: ProductRouterHarness[] = [];
    const ownerOptions = capture<Readonly<FilePlaybackProductHostMediaOwnerOptions>>();
    const publishSourceLease = vi.fn(
      async (authority: Readonly<FilePlaybackProductHostLocalTrackWarmResult>) => {
        const resolver = requireCapture(
          ownerOptions,
          'late-ready warm owner options unavailable',
        ).resolveWarmPeerRangeSource;
    ...
    expect(requireCapture(ownerOptions, 'late-ready warm owner options unavailable').hostRoom).toBe(
      setup.hostRooms[0]!.port,
    );
  });
  ...
  it('fences a same-queue in-flight warm before prepared publication and retires its late lease', async () => {
    ...
    const warmSignal = capture<AbortSignal>();
    const capability = freezeCanonical({
    ...
    const owner = Object.freeze({
      ...
      publishPrepared: vi.fn(async (prepared: Readonly<HostPreparedLocalTrack>) => {
        expect(requireCapture(warmSignal, 'same-queue warm signal unavailable').aborted).toBe(true);
        return freezeCanonical({ schemaVersion: 1, prepared }) as never;
      }),
      bindPrepared: vi.fn(
        async (prepared: Readonly<HostPreparedLocalTrack>) =>
  ...
  it('wires the exact active host room, shared publisher, health callback, and failed-notification close', async () => {
    const routers: ProductRouterHarness[] = [];
    const publisher = capture<FilePlaybackR2WholeBlobPublisher>();
    const hostOwnerOptions = capture<Readonly<FilePlaybackProductHostMediaOwnerOptions>>();
    ...
    const setup = harness({
      ...
      mediaFactoriesForTests: {
        ...
        createHostPublisher: (roomToken) => {
          const created = new FilePlaybackR2WholeBlobPublisher({ roomToken });
          publisher.value = created;
          vi.spyOn(created, 'close');
          return created;
        },
        createHostMediaOwner: (options) => {
          hostOwnerOptions.value = options;
          return hostOwner;
        },
      },
    });
    ...
    const exactOwnerOptions = requireCapture(hostOwnerOptions, 'host owner options unavailable');
    const exactPublisher = requireCapture(publisher, 'host publisher unavailable');
    expect(exactOwnerOptions.context).toBe(context);
    expect(exactOwnerOptions.hostRoom).toBe(setup.hostRooms[0]?.port);
    expect(exactOwnerOptions.publisher).toBe(exactPublisher);
    expect(exactOwnerOptions).not.toHaveProperty('boundedRoutePolicy');
    expect(wrappedOwner).not.toBe(hostOwner);
    expect(exactOwnerOptions.sendRequired(peer, freezeCanonical({ type: 'fixture' }))).toBe(true);
    expect(setup.sessions.sendRequired).toHaveBeenCalledWith(peer, { type: 'fixture' });

    exactOwnerOptions.closeConnection(peer);
    expect(setup.sessions.closeConnection).not.toHaveBeenCalled();
    await Promise.resolve();
    ...
    exactOwnerOptions.onHealthSystemMessage(healthMessage);
    ...
    expect(exactPublisher.close).toHaveBeenCalledOnce();
  });
  ...
  it('wires one guest room registry/manager and fail-closes exact room resources', async () => {
    ...
    const registry = capture<FilePlaybackAssetRegistry>();
    const registryRoomToken = capture<object>();
    const registryFatal = capture<(token: object, error: Error) => void>();
    ...
    const setup = harness({
      mediaFactoriesForTests: {
        ...
        createGuestRegistry: (roomToken, onFatalRoom) => {
          registryRoomToken.value = roomToken;
          registryFatal.value = onFatalRoom;
          const created = new FilePlaybackAssetRegistry({
        ...
        createGuestMediaOwner: (options) => {
          guestOwnerOptions.value = options;
          return guestOwner;
        },
      },
    });
    ...
    const exactGuestOwnerOptions = requireCapture(
      guestOwnerOptions,
      'guest owner options unavailable',
    );
    const exactRegistry = requireCapture(registry, 'guest registry unavailable');
    const exactRegistryRoomToken = requireCapture(
      registryRoomToken,
      'guest registry room token unavailable',
    );
    expect(exactGuestOwnerOptions.context).toBe(context);
    expect(exactGuestOwnerOptions.registry).toBe(exactRegistry);
    expect(exactGuestOwnerOptions.manager).toBe(manager);
    expect(exactGuestOwnerOptions.roomToken).toBe(exactRegistryRoomToken);
    expect(exactGuestOwnerOptions).not.toHaveProperty('boundedRoutePolicy');
    ...
    expect(exactGuestOwnerOptions.maxEncodedSize).toBe(5 * 1024 * 1024 * 1024);
    expect(exactGuestOwnerOptions.maxEncodedSize).toBeGreaterThan(REMOTE_SHARE_MAX_BYTES);
    const offerToken = Object.freeze({});
    const offers = new FileMediaOfferRegistry({
      liveConnectionToken: offerToken,
      sessionId: context.sessionId,
      connectionId: context.connectionId,
      maxEncodedSize: exactGuestOwnerOptions.maxEncodedSize,
      nowRoomTimeMs: () => 1_000,
    ...
    expect(
      exactGuestOwnerOptions.sendRequired(context, freezeCanonical({ type: 'guest-frame' })),
    ).toBe(true);
    expect(setup.sessions.sendRequired).toHaveBeenCalledWith(peer, { type: 'guest-frame' });
    expect(
      exactGuestOwnerOptions.sendRequired(
    ...
    const peerControl = createPeerRangeCloseHandleFrame({
      connectionId: context.connectionId,
      sourceIdentity: 'runtime-peer-source',
      handleId: 'runtime-peer-handle',
    });
    const sendsBeforePeerGate = vi.mocked(setup.sessions.sendRequired).mock.calls.length;
    ...
    expect(exactGuestOwnerOptions.sendRequired(context, peerControl)).toBe(true);
    ...
    expect(loadingPending).toHaveBeenCalledWith({
      owner: 'guest-prepare',
      token: 'guest-runtime-loading',
    });
    ...
    expect(loadingSettled).toHaveBeenCalledWith({
      owner: 'guest-prepare',
      token: 'guest-runtime-loading',
    });
    exactGuestOwnerOptions.onPauseGateStateChange?.({
    ...
    Reflect.apply(
      ...
      undefined,
      [exactRegistryRoomToken, new Error('registry fatal')],
    );
    await Promise.resolve();
    ...
    expect(exactRegistry.close).toHaveBeenCalledWith(exactRegistryRoomToken);
  });
  ...
  it('pins one opt-in bounded route policy across host and guest room owners', () => {
    ...
    expect(
      requireCapture(hostOwnerOptions, 'bounded host owner options unavailable').boundedRoutePolicy,
    ...
    expect(
      requireCapture(guestOwnerOptions, 'bounded guest owner options unavailable')
        .boundedRoutePolicy,
    ).toBe(FILE_PLAYBACK_MP3_M4A_V1_BOUNDED_ROUTE_POLICY);
    setup.runtime.endRoom();
  });
