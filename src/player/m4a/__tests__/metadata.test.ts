...
type MutableManifestWire = Mutable<ReturnType<typeof snapshotM4aAacLcManifest>>;
...
describe('bounded M4A AAC-LC manifest assembly', () => {
  ...
  it('treats brands as diagnostic data instead of a codec admission gate', async () => {
    ...
    const clone = structuredClone(snapshotM4aAacLcManifest(manifest)) as MutableManifestWire;
...
describe('M4A AAC-LC structured manifest validation', () => {
  async function freshWire(): Promise<MutableManifestWire> {
    const manifest = await readM4aAacLcMetadata(buildM4aAacFixture().source, signal());
    return structuredClone(snapshotM4aAacLcManifest(manifest)) as MutableManifestWire;
  }

  it.each([
    [
      'an extra top-level field',
      (value: MutableManifestWire) => {
    ...
    [
      'a symbol field',
      (value: MutableManifestWire) => {
        Reflect.set(value, Symbol('extra'), true);
      },
    ],
    [
