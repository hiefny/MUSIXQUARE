...
import type { FilePlaybackAuxiliaryAdoptionEvent } from '../../network/file-playback-application-session.ts';
import {
...
function isAuxiliaryPrimitive(value: unknown): value is string | number | boolean | null {
  return (
    value === null ||
    typeof value === 'string' ||
    typeof value === 'number' ||
...
function isAuxiliaryFrame(value: unknown): value is FilePlaybackAuxiliaryAdoptionEvent['frame'] {
...
function requireAuxiliaryFrame(value: unknown): FilePlaybackAuxiliaryAdoptionEvent['frame'] {
  if (!isAuxiliaryFrame(value)) {
    throw new Error('Expected an auxiliary frame with primitive values');
  }
  return value;
}
          ...
          sendRequired: (_connection, frame) => {
            ...
            if (type === 'chunk' || type === 'error') {
              ...
              guestOwner.adoptAuxiliaryMessage(
                freezeCanonical({
                  frame: requireAuxiliaryFrame(frame),
                  connection: pair.guestConnection,
                  channel: pair.guestChannel,
                  connectionToken: pair.guestConnection,
                }),
