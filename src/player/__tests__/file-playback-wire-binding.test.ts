...
describe('file playback wire binding rendezvous successors', () => {
  it('admits STOP over only the exact marked unprepared baseline', () => {
    const registry = new FilePlaybackWireBindingRegistry();
    registry.bootstrapStopped(6);
    const baselineLease = registry.stageUnpreparedBaselineMedia(CURRENT_BINDING);

    const stopLease = registry.admitRemoteSuccessor(
      SUCCESSOR_REFERENCE,
      CURRENT_BINDING.run,
      'stop',
    );
    expect(registry.resolveStopSuccessor(SUCCESSOR_REFERENCE, CURRENT_BINDING.run)).toEqual({
      status: 'active',
      stateLease: stopLease,
    });
    registry.commitStop(stopLease, CURRENT_BINDING.run);
    expect(registry.resolveState(CURRENT_REFERENCE)).toEqual({ status: 'stale' });
    expect(registry.resolveState(SUCCESSOR_REFERENCE)).toEqual({ status: 'stale' });
    expect(() => registry.bindingForStateLease(baselineLease)).toThrow(/retired/u);

    const ordinary = new FilePlaybackWireBindingRegistry();
    ordinary.bootstrapStopped(6);
    ordinary.stageMedia(CURRENT_BINDING);
    expect(() =>
      ordinary.admitRemoteSuccessor(SUCCESSOR_REFERENCE, CURRENT_BINDING.run, 'stop'),
