...
| URL credential privacy               | In browser network tools, verify a new PRO socket has no `ticket` query, offers the stable `mxqr.pro-signaling.v1` marker plus the bearer protocol token, and receives only the stable marker. Before the cutoff, a controlled legacy request increments only `pro_ticket_legacy_query_used`; at/after the cutoff it receives the no-store 426 refresh contract and increments only `pro_ticket_legacy_query_update_required`. Verify no metric or custom log contains the URL, ticket, room, participant, IP, or User-Agent.                       |
| Platform observability               | Confirm all six production Wrangler configs keep sampled custom logs enabled while `invocation_logs = false` and automatic traces disabled. Signaling may be reconsidered only after its query detector is removed under [`cloudflare/pro-signaling-query-ticket-cutover.md`](../cloudflare/pro-signaling-query-ticket-cutover.md); legal copy and policy tests must be updated before any telemetry behavior changes. Inspect a controlled OAuth callback and PRO join and verify no credential-bearing full URL appears in the custom log schema. |
