...
function isTestOrSnapshot(path: string): boolean {
  return (
...
export function isBrandCopySource(path: string): boolean {
  ...
  if (normalized === 'scripts/check-brand-capitalization.mts') return false;
...
function isFixedTechnicalSpelling(source: string, match: RegExpExecArray): boolean {
...
export function findBrandCapitalizationViolations(
  ...
  const violations: BrandCapitalizationViolation[] = [];

  for (const match of source.matchAll(PRODUCT_NAME_PATTERN)) {
    const spelling = match[0];
    if (spelling === undefined) continue;
    if (spelling === 'MUSIXQUARE' || isFixedTechnicalSpelling(source, match)) continue;
    const line = source.slice(0, match.index ?? 0).split('\n').length;
    violations.push({
      path,
      line,
      spelling,
      message: `${path}:${line}: use MUSIXQUARE, found ${JSON.stringify(spelling)}`,
    });
  }
...
export function listBrandCopySources(repository = process.cwd()): string[] {
...
export function checkBrandCapitalization(
