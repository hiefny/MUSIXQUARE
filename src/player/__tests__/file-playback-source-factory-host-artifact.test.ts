...
function boundedSource<Backend extends 'audio-buffer' | 'bounded-stream' = 'bounded-stream'>(
  queueItemId: QueueItemId,
  backend: Backend = 'bounded-stream' as Backend,
) {
  return {
    backend,
    queueItemId,
...
describe('file playback source factory host timeline artifact', () => {
  ...
  it('returns null for absent binding and for the ordinary AudioBuffer route', async () => {
    ...
    const blob = new File([mp3Bytes().slice().buffer], 'ordinary.mp3', { type: 'audio/mpeg' });
